...
RPG built in Unity 6.5 (mobile kept as a viable later port).** Read this file first, every session. It is short on purpose and
...
- **Engine:** Unity **6.5** (6000.5.x), **2D URP**, **C#**. **PC-first** (Windows/desktop;
...
- **Editor-as-a-tool (Unity CLI + Pipeline + MCP):** `docs/tooling/unity-cli-and-mcp.md` + skill `.claude/skills/unity-cli` — drive the OPEN editor (`unity command …`), run tests (`unity test`); lanes without an editor keep the no-Unity paths
