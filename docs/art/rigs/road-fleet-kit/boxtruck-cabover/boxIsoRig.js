...
(function (root) {
  ...
  function buildDoors(out,s){for(const [sx,pose]of [[-1,s.dL],[1,s.dR]]){const a=sx*pose*65*DEG;part(out,T=>p2DoorLeaf(T,sx,G.doorY,G.hwCab,G.doorZ0,1.38,G.doorHead,s),p=>hingeZ(p,sx*0.94,G.doorY[1],Math.cos(a),Math.sin(a)));}}

  // ---- the tilt cab: shell, glass, flares, steps, mirrors, interior — one hinged group ----
  function originalCab(out,s){
    ...
    fitHeader(T);
      ...
      p2Roof(T,G.hwCabRoof,G.cabBackY+.02,3.10,G.cabRoofZ);
  ...
  function originalBox(out,s){
    ...
    p2CargoRoof(out,G.hwBox,G.boxR,G.boxF,G.boxRoofZ);
  ...
  const FOLD = { alloy:'chrome', p2Plate:'chrome', dash:'rubber' };
  ...
  function buildBox(out,s){originalBox(out,s);artCargoRails(out,G.hwBox,G.boxR,G.boxF,.90,G.boxRoofZ);}

  function buildCab(out,s){const T=[];originalCab(T,{...s,tilt:0});
    const keep=T.filter(f=>!(!f.fitGroup&&f.v.every(p=>p[1]>3.13)&&['paint','glass','grille','head','glow'].includes(f.mat)));
    const faceStart=keep.length;
    p2Y(keep,G.noseY,p2Rect(-.98,.98,.48,1.30,.10),'paint',.04);
    p2Intake(keep,G.noseY+.022,.48,.72,1.06,false);
    for(const sx of [-1,1])p2Lamp(keep,sx,G.noseY+.02,.53,.93,.91,1.17,false,s.night);
    const ws=F([[.84,G.wsB.y,G.wsB.z],[-.84,G.wsB.y,G.wsB.z],[-.80,G.wsT.y,G.wsT.z],[.80,G.wsT.y,G.wsT.z]],'glass',0);p2Glaze(keep,ws);
    for(const sx of [-1,1])p2Face(keep,[[sx*.84,G.wsB.y,1.28],[sx*.98,G.wsB.y,1.28],[sx*.92,G.wsT.y,2.14],[sx*.80,G.wsT.y,2.14]],'paint',.02,[0,1,0]);
    // The fitted header from originalCab is retained.
    const a=-s.tilt*G.tiltDeg*DEG;
    for(const f of keep){f.v=f.v.map(([x,y,z])=>{const t=Math.max(0,Math.min(1,(y-3.02)/.26));return rotX([x,y-(f.fitGroup==='door'?0:.065*t*Math.pow(Math.abs(x)/.98,4)),z],G.tiltHinge.y,G.tiltHinge.z,Math.cos(a),Math.sin(a));});out.push(f);}
  }
  ...
  function buildRollup(out,s){p1Rollup(out,s);const u=.08+s.rollup*G.slats*G.slatH,p=rollPath(u);
    if(u<=G.vTrack){
      boxAt(out,-.16,.16,p.y-.028,p.y-.008,p.z-.035,p.z+.035,'rubber',-.1);
      bar(out,[-.105,p.y-.05,p.z],[.105,p.y-.05,p.z],.018,'p2Bright',.10);
    }else{
      boxAt(out,-.16,.16,p.y-.035,p.y+.035,p.z-.028,p.z-.008,'rubber',-.1);
      bar(out,[-.105,p.y,p.z-.05],[.105,p.y,p.z-.05],.018,'p2Bright',.10);
    }
  }


  // A single metric envelope defines the cab skin, door, jamb, and roof junction.
  const FIT={"belt":1.38,"rake":0.12,"front":3.28,"roofY":3.1,"roofZ":2.3,"drop":0.065,"xb":0.98,"xt":0.92,"hx":0.94,"hingeSetback":0.065};
  ...
  root.BoxIso = { cabFit, W, H, PX, DIRS:8, pivot:{x:cx,y:groundY}, defaultElev:DEFAULT_ELEV,
    ...
    mesh:(opts)=>build(resolve(opts||{})), list, dims, resolve, render, frames, anchors, project, gatePose, rollPath };
