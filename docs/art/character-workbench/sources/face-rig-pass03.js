/* Hidden Harbours — head study 03. Original HeadIso is required for customization geometry.
...
(function(root){
 const C={headScale:.97,pixelPhaseX:.5,eyeX:.073,eyeZ:.053,eyeW:.067,eyeH:.037,browZ:.098,
 ...
 function createHead(build,hc){
  ...
  const p=[[-.016,.106,.012],[.016,.106,.012],[.023,.109,-.044],[-.023,.109,-.044],[0,.146,-.024]];
 ...
 function sample(u,v,state,build){
  ...
  for(const side of [-1,1]){
   ...
   if(Math.abs(dx)<=half){
    if(lid>.87){if(Math.abs(z-(bottom+height*.25))<.010)return 1;}
     ...
     const pupilX=gx*(half-.014),pupilZ=bottom+open*.47+gy*.008;
     if(Math.abs(dx-pupilX)<.013&&Math.abs(z-pupilZ)<.017)return 1;
     if(Math.abs(dx-pupilX)<.016&&Math.abs(z-pupilZ)<.019)return 3;
     return 2;
    }
   }
   const brow=BROWS[b.browShape||'natural'];
   if(brow&&Math.abs(dx)<half+.005){
    const tilt=clamp((st.brow||0)+brow.tilt,-1.5,1.5),y=C.browZ+socketCurve+brow.lift+(st.raise||0)*.014+side*dx*tilt*.52;
  ...
  if(Math.abs(mx)<.070&&Math.abs(mz)<.060){
   if(mouth==='neutral'||mouth==='closed'){if(Math.abs(mx)<.030&&Math.abs(mz)<lip)return 5;}
