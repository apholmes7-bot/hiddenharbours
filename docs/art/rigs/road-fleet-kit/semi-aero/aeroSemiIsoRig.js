/* Hidden Harbours — parametric ISO ROAD-VEHICLE rig, AERO SLEEPER SEMI TRACTOR (same turntable +
   camera + shading as vehicleIsoRig.js / convBoxIsoRig.js / the fleet). Body: AERO SEMI — a modern
   aero sleeper tractor (Cascadia/VNL class): raked hood, integrated roof fairing to 3.85 m, side
   skirts, 1.63 m sleeper, tandem drive axles, and a FIFTH WHEEL — the coupling half of the
   tractor+trailer pairs (trailers are separate bodies with kingpin anchors; the GAME articulates
   the pair). 45deg steps, elev 40deg, flat-facet shading from the fixed upper-LEFT key, z-buffered,
   ordered dither, NO AA, 32 px = 1 m, ringless from birth (ADR 0031).
   Cell 384 x 320 @ 192,214 — the road cell; nothing on a bare tractor overhangs it.

   ARTICULATION — pose params on render(dir,opts), 0..1 unless noted:
     dL dR             cab doors, hinged on their FORWARD edge, 0 -> 65deg
     hood              the whole FRONT CLIP — hood, fenders, arches, grille, headlamps — tilts
                       forward 0 -> 72deg about a hinge at the bumper line and bares the engine.
     roll              master wheel roll, REVOLUTIONS (cyclic); wFL wFR wRL wRR per-CORNER offsets
                       (each tandem side shares its corner's roll, like the box trucks' duals)
     susF susR         suspension travel per axle group, -1..1 (the BODY moves; wheels stay down)
     steer             front pair yaw, Ackermann-split, -1..1; +1 is full LEFT lock. Inner 32deg —
                       the painted fenders (half-width 1.27 m) are the envelope.
     yaw               heading off the 45deg grid, DEGREES (-45..45), rebaked under the fixed key.
   Parts (not poses): mirrors, mudflaps, skirts (true|false — fleet side fairings; false bares the
   frame, tanks and battery boxes between the axles).

   COUPLING (the tier's whole point): the fifth wheel plate tops at z 1.18, centred y -2.40 (0.10 m
   aft of the tandem centre), slot opening AFT. Kingpin-to-cab-back is 1.52 m: a 2.44 m-wide trailer
   with the pack's 0.90 m kingpin set swings a 1.516 m nose radius — full jackknife clears by 4 mm.
   anchors().fifthWheel is the coupling point; the sidecar TOW block carries the arithmetic.

   ORIGIN / PIVOT: ground-centre of the body footprint. +x curb side, +y nose, +z up.

   Exposes globalThis.AeroSemiIso = { W,H,PX,DIRS,pivot,order,defaultElev, BODY,TRIM,IRON,GALV,
...
(function (root) {
  ...
  // c = 0.1083 m puts 29.008 stripe periods on the 3.1416 m circumference: one revolution closes to a
  // sub-pixel phase, while a HALF revolution lands mid-stripe (14.5) — invisible seam, visible motion.

  // ================= GEOMETRY — AERO SLEEPER SEMI =================
  const TF=0.09, TR=0.11;                          // suspension travel per axle group, metres
  const G = {
    noseY:3.95, bumpF:[3.97,4.12], frameR:-3.45,
    cowlY:2.02, cabBackY:-0.88, axF:2.95, tandA:-1.70, tandB:-2.90, axR:-2.30,  // axR = tandem centre
    wheelR:0.50, tireW:0.30, frontWX:0.84, dualXi:0.60, dualXo:0.90,
    hwCab:1.22, hwCabRoof:1.14, cabRoofZ:2.86, cabFloorZ:1.15,
    hwHood:0.86, hoodZc:1.74, hoodZn:1.32, hoodY0:2.08, hoodY1:3.93,
    hwFender:1.27, fenderTopZ:1.34, hoodHinge:{y:4.02,z:0.55}, hoodDeg:72,
    wsB:{y:2.00,z:1.80}, wsT:{y:1.66,z:2.66},
    doorY:[0.75,1.77], doorZ0:1.00, doorHead:2.42,
    archF:{r:0.68, zc:0.44}, archFy:[2.20,3.70],
    fair:{yFront:1.50, yMid:0.45, zMid:3.60, zTop:3.85, hw:0.95},
    fw:{y:-2.40, plate:[-2.80,-2.00], hw:0.45, topZ:1.18, rampTo:{y:-3.02,z:1.00}},
    skirt:{y0:-1.15, y1:2.28, z0:0.32, z1:1.06, hw:1.20},
    tank:{x:0.88, r:0.30, y:[-0.10,1.30], z:0.62},
  };

  const BODIES = {
    aeroSemi: { key:'aeroSemi', label:'Aero Sleeper Semi', kind:'semi_tractor',
      loa:+(G.bumpF[1]-G.frameR).toFixed(2), bodyL:+(G.noseY-G.frameR).toFixed(2),
      width:+(G.hwFender*2).toFixed(2), bodyW:+(G.hwCab*2).toFixed(2),
      height:+(G.fair.zTop+0.04).toFixed(2), wheelbase:+(G.axF-G.axR).toFixed(2),
  ...
  const STEER_MAX = 32;                                   // inner wheel, degrees, at full lock
  ...
  const PRESETS = {
    ...
    fleet:       { paint:'white', weather:0.45 },
    harbourLine: { paint:'teal',  weather:0.30 },
    ownerOp:     { paint:'red',   weather:0.35 },
    longHaul:    { paint:'blue',  weather:0.28, night:true },
  };
  const CUES = {
    doors: (t)=>({ dL:t, dR:t }),
    hood:  (t)=>({ hood:t }),
  ...
  function resolve(opts){
    ...
    return {
      body:'aeroSemi', B:BODIES.aeroSemi,
      ...
      mirrors:g('mirrors',true), mudflaps:g('mudflaps',true), skirts:g('skirts',true),
      night:!!opts.night, outline: opts.outline!=null?!!opts.outline:KEYLINE_DEFAULT };
  }
  function dims(opts){ const s=resolve(opts);
    return Object.assign({ travelF:TF, travelR:TR }, s.B, { skirts:s.skirts }); }
  ...
  // ---- chassis: frame, tanks, steps, deck, fifth wheel, engine (bared by the hood) ----
  function buildFrame(out,s){
    for(const sx of [-1,1]) boxAt(out, sx*0.47-0.045, sx*0.47+0.045, G.frameR, 3.80, 0.55, 0.95, 'iron', -0.15);
    for(const y of [-3.30,-2.30,-1.20,-0.20,0.90,2.00,3.10]) boxAt(out,-0.47,0.47,y-0.05,y+0.05,0.55,0.68,'iron',-0.3);
    for(const sx of [-1,1]){                                                   // fuel tanks both sides
      tube(out,[sx*G.tank.x,G.tank.y[0],G.tank.z],[sx*G.tank.x,G.tank.y[1],G.tank.z],G.tank.r,12,'galv',-0.1,true);
      boxAt(out, Math.min(sx*0.84,sx*1.16), Math.max(sx*0.84,sx*1.16), 0.95, 1.35, 0.44, 0.50, 'iron', -0.2);   // treads
      boxAt(out, Math.min(sx*0.86,sx*1.16), Math.max(sx*0.86,sx*1.16), 0.95, 1.35, 0.80, 0.86, 'iron', -0.15);
      bar(out, [sx*1.00,1.15,0.50],[sx*1.00,1.15,0.80],0.022,'iron',-0.4);
    }
    boxAt(out, -1.12,-0.82, -0.55, 0.05, 0.44, 0.86, 'rubber', -0.25);         // battery box, street aft of tank
    boxAt(out, 0.82,1.12, -0.75, -0.15, 0.42, 0.80, 'galv', -0.3);             // DPF/after-treatment, curb
    tube(out,[-0.60,-3.20,0.30],[-0.60,-3.40,0.28],0.048,8,'chrome',0.15);     // exhaust tip, under-frame street
    boxAt(out, -1.24,1.24, G.bumpF[0], G.bumpF[1], 0.30, 0.68, 'paint', 0.02); // aero painted bumper (stays put)
    for(const sx of [-1,1]) bar(out,[sx*0.30,4.06,0.42],[sx*0.30,4.14,0.42],0.035,'iron',-0.2);  // tow pins
    for(const sx of [-1,1])                                                    // frame-end lamp bar
      boxAt(out, Math.min(sx*0.30,sx*0.46), Math.max(sx*0.30,sx*0.46), -3.45, -3.40, 0.75, 0.88, 'lensR', 0.25);
    if(s.mudflaps) for(const sx of [-1,1]){
      wallY(out, -3.30, Math.min(sx*0.52,sx*1.08), Math.max(sx*0.52,sx*1.08), 0.06, 0.50, 'rubber', -0.4, -1);
      wallY(out, -3.295, Math.min(sx*0.52,sx*1.08), Math.max(sx*0.52,sx*1.08), 0.06, 0.50, 'rubber', -0.8, +1);
    }
    // deck plates (catwalk) between cab and fifth wheel + glad-hand lines on the back wall
    slab(out, [[-0.55,-1.95],[0.55,-1.95],[0.55,-0.92],[-0.55,-0.92]], 1.10, 'galv', -0.1, gridTex());
    for(const sx of [-1,1]) bar(out,[sx*0.22,-0.90,1.98],[sx*0.30,-1.55,1.30],0.020,'rubber',-0.35);   // air lines
    boxAt(out, -0.30,0.30, -0.92,-0.88, 1.88, 2.06, 'iron', -0.2);             // glad-hand panel
    // fifth wheel: pedestal, plate with aft slot, approach ramps, release handle
    boxAt(out, -0.35,0.35, -2.55,-2.25, 0.95, 1.13, 'iron', -0.3);
    slab(out, [[-G.fw.hw,G.fw.plate[0]],[G.fw.hw,G.fw.plate[0]],[G.fw.hw,G.fw.plate[1]],[-G.fw.hw,G.fw.plate[1]]], G.fw.topZ, 'galv', 0.15);
    slab(out, [[-0.06,G.fw.plate[0]-0.001],[0.06,G.fw.plate[0]-0.001],[0.06,G.fw.y],[-0.06,G.fw.y]], G.fw.topZ+0.004, 'shade', -0.6);  // kingpin slot, opens aft
    ...
    bar(out, [-0.45,-2.40,1.10],[-0.78,-2.40,1.08],0.022,'iron',-0.25);        // release handle, street
    // engine bay (bared by the tilted clip)
    wallY(out, 2.04, -0.84, 0.84, 0.60, 1.65, 'shade', -0.8, +1);              // firewall
    wallX(out, 0.80, 2.06, 3.86, 0.62, 1.30, 'shade', -0.8, -1);
    wallX(out,-0.80, 2.06, 3.86, 0.62, 1.30, 'shade', -0.8, +1);
    boxAt(out, -0.66,0.66, 3.86, 3.92, 0.55, 1.24, 'shade', -0.6);             // radiator wall
    boxAt(out, -0.38,0.38, 2.25, 3.40, 0.55, 1.18, 'iron', -0.3);              // block
    boxAt(out, -0.30,0.30, 2.35, 3.30, 1.18, 1.30, 'galv', -0.1);              // valve cover
    tube(out,[0.58,2.55,1.05],[0.58,2.95,1.05],0.15,10,'rubber',-0.2);         // air cleaner, curb
    for(const sx of [-1,1]) boxAt(out, Math.min(sx*0.88,sx*0.96), Math.max(sx*0.88,sx*0.96), 2.04, 2.10, 1.56, 1.64, 'trim', 0.3);  // hood latches
    // side skirts (part) — fleet chassis fairings between the axles
    if(s.skirts){ const wear=wearTex(s.weather);
      for(const sx of [-1,1]){
        texWallX(out, sx*G.skirt.hw, G.skirt.y0, G.skirt.y1, G.skirt.z0, G.skirt.z1, 'paint', sx>0?0.16:-0.44, sx, wear);
        quad(out, sx>0?[G.skirt.hw,G.skirt.y0,G.skirt.z0]:[-G.skirt.hw,G.skirt.y1,G.skirt.z0],
                  sx>0?[G.skirt.hw,G.skirt.y1,G.skirt.z0]:[-G.skirt.hw,G.skirt.y0,G.skirt.z0],
                  sx>0?[G.skirt.hw-0.06,G.skirt.y1,G.skirt.z0-0.10]:[-G.skirt.hw+0.06,G.skirt.y0,G.skirt.z0-0.10],
                  sx>0?[G.skirt.hw-0.06,G.skirt.y0,G.skirt.z0-0.10]:[-G.skirt.hw+0.06,G.skirt.y1,G.skirt.z0-0.10], 'paint', -0.15);
        slab(out, sx>0?[[G.skirt.hw-0.04,G.skirt.y0],[G.skirt.hw,G.skirt.y0],[G.skirt.hw,G.skirt.y1],[G.skirt.hw-0.04,G.skirt.y1]]
                     :[[-G.skirt.hw,G.skirt.y0],[-G.skirt.hw+0.04,G.skirt.y0],[-G.skirt.hw+0.04,G.skirt.y1],[-G.skirt.hw,G.skirt.y1]],
             G.skirt.z1, 'paint', 0.20);
      }
    }
  }

  // ---- the front clip: raked aero hood, fenders, grille, headlamps — one hinged group ----
  function buildHoodClip(out,s){
    ...
    part(out,(T)=>{
      ...
      wallY(T, y1, -G.hwHood, G.hwHood, 1.26, zn-0.02, 'paint', 0.0, +1);
      for(const sx of [-1,1]){
        if(sx>0) quad(T, [0.86,y0,1.72],[1.06,y0,G.fenderTopZ],[1.06,y1,1.28],[0.86,y1,1.30], 'paint', 0.24);
        else     quad(T, [-1.06,y0,G.fenderTopZ],[-0.86,y0,1.72],[-0.86,y1,1.30],[-1.06,y1,1.28], 'paint', 0.24);
        quad(T, sx>0?[1.06,y0,G.fenderTopZ]:[-G.hwFender,y0,G.fenderTopZ],
                sx>0?[G.hwFender,y0,G.fenderTopZ]:[-1.06,y0,G.fenderTopZ],
                sx>0?[G.hwFender,y1-0.02,1.28]:[-1.06,y1-0.02,1.28],
                sx>0?[1.06,y1-0.02,1.28]:[-G.hwFender,y1-0.02,1.28], 'paint', 0.30, 0, null, wear);
        archPanel(T, sx*G.hwFender, G.archFy[0], G.archFy[1], 0.46, 1.32, 'paint', sx>0?0.18:-0.42, sx,
          [{yc:G.axF, r:G.archF.r, zc:G.archF.zc}], wear);
        wallX(T, sx*1.02, G.axF-G.archF.r, G.axF+G.archF.r, 0.96, 1.12, 'shade', -0.85, sx);
        boxAt(T, Math.min(sx*0.66,sx*1.06), Math.max(sx*0.66,sx*1.06), 3.91, 3.96, 0.98, 1.16, s.night?'glow':'head', 0.35);
        wallY(T, y1, Math.min(sx*0.66,sx*1.06), Math.max(sx*0.66,sx*1.06), 0.46, 0.98, 'paint', 0.05, +1, null, wear);
        wallY(T, y1, Math.min(sx*0.66,sx*1.06), Math.max(sx*0.66,sx*1.06), 1.16, 1.26, 'paint', 0.05, +1);
        wallY(T, y1, Math.min(sx*0.62,sx*0.66), Math.max(sx*0.62,sx*0.66), 0.46, 1.26, 'paint', 0.05, +1);
        wallY(T, y1, Math.min(sx*1.06,sx*G.hwFender), Math.max(sx*1.06,sx*G.hwFender), 0.46, 1.28, 'paint', 0.05, +1, null, wear);
      }
      T.push(F([[0.62,3.96,0.70],[-0.62,3.96,0.70],[-0.62,3.96,1.26],[0.62,3.96,1.26]],'grille',-0.1,0,
        [[0.62,0.70],[-0.62,0.70],[-0.62,1.26],[0.62,1.26]], grilleTex()));
      wallY(T, y1, -0.62, 0.62, 0.46, 0.70, 'paint', 0.05, +1);
    }, s.hood>0 ? (p)=>rotX(p, G.hoodHinge.y, G.hoodHinge.z, ca, sa) : null);
  }

  // ---- cab + sleeper: static — shell, glass, doors, mirrors, fairing, interior ----
  function buildDoors(T,s){
    ...
    for(const d of [ {sx:+1, pose:s.dR}, {sx:-1, pose:s.dL} ]){
      ...
      part(T,(D)=>{
        texWallX(D, sx*G.hwCab, y0, y1, G.doorZ0, 1.62, 'paint', sx>0?0.18:-0.42, sx, wear);
        wallX(D, sx*G.hwCab, y0, y1, 2.36, G.doorHead, 'paint', sx>0?0.10:-0.48, sx);
        wallX(D, sx*1.232, y0+0.06, y1-0.06, 1.62, 2.36, 'glass', sx>0?-0.15:-0.55, sx);
        wallX(D, sx*1.14, y0+0.02, y1-0.02, G.doorZ0+0.04, G.doorHead-0.04, 'leaf', -0.7, -sx);
        boxAt(D, Math.min(sx*1.23,sx*1.26), Math.max(sx*1.23,sx*1.26), y0+0.10, y0+0.30, 1.42, 1.48, 'trim', 0.3);
      }, (p)=>hingeZ(p, sx*1.18, y1, ca, sa));
    }
  }
  function buildCab(out,s){
    const wear=wearTex(s.weather);
    quad(out, [1.04,G.wsB.y,G.wsB.z],[-1.04,G.wsB.y,G.wsB.z],[-0.96,G.wsT.y,G.wsT.z],[0.96,G.wsT.y,G.wsT.z], 'glass', -0.18);
    ...
    quad(out, [1.08,G.wsT.y,G.wsT.z],[-1.08,G.wsT.y,G.wsT.z],[-G.hwCabRoof,1.50,2.84],[G.hwCabRoof,1.50,2.84], 'paint', 0.28);
    slab(out, [[-1.18,2.00],[1.18,2.00],[1.18,2.08],[-1.18,2.08]], 1.72, 'paint', 0.12);        // cowl strip
    wallY(out, G.cowlY, -1.22, 1.22, 1.00, 1.72, 'paint', 0.08, +1);                             // cowl face
    boxAt(out, -G.hwCabRoof, G.hwCabRoof, G.cabBackY, 1.50, 2.80, G.cabRoofZ, 'paint', 0.06, false, wear);
    for(const sx of [-1,1]){
      wallX(out, sx*G.hwCab, G.cabBackY, G.cowlY, 0.62, G.doorZ0, 'paint', sx>0?0.16:-0.44, sx, null, wear);   // skirt band
      ...
      if(sx>0) quad(out, [1.22,G.cabBackY,G.doorHead],[1.22,2.02,G.doorHead],[1.14,2.02,2.82],[1.14,G.cabBackY,2.82], 'paint', 0.12);
      else     quad(out, [-1.22,2.02,G.doorHead],[-1.22,G.cabBackY,G.doorHead],[-1.14,G.cabBackY,2.82],[-1.14,2.02,2.82], 'paint', -0.46);
      // sleeper window, set-in
      wallX(out, sx*1.232, -0.60, -0.10, 1.75, 2.20, 'glass', sx>0?-0.12:-0.50, sx);
      // cab side extenders at the back edge (aero cue)
      boxAt(out, Math.min(sx*1.16,sx*1.22), Math.max(sx*1.16,sx*1.22), G.cabBackY-0.16, G.cabBackY+0.02, 1.15, 3.50, 'paint', sx>0?0.08:-0.40, false, wear);
      if(s.mirrors){
        bar(out,[sx*1.19,1.72,2.36],[sx*1.42,1.86,2.28],0.020,'iron',-0.1);
        bar(out,[sx*1.19,1.72,1.98],[sx*1.42,1.86,2.04],0.018,'iron',-0.15);
        boxAt(out, Math.min(sx*1.38,sx*1.48), Math.max(sx*1.38,sx*1.48), 1.80, 1.90, 1.64, 2.06, 'dash', -0.05);
        wallY(out, 1.80, Math.min(sx*1.39,sx*1.47), Math.max(sx*1.39,sx*1.47), 1.68, 2.02, 'glass', -0.25, -1);
      }
    }
    wallY(out, G.cabBackY, -1.22, 1.22, 0.95, 2.82, 'paint', -0.30, -1, null, wear);            // back wall
    // roof fairing: two slopes to 3.85, sides, open aft face
    const fw=G.fair.hw;
    quad(out, [fw,G.fair.yFront,2.88],[-fw,G.fair.yFront,2.88],[-fw,G.fair.yMid,G.fair.zMid],[fw,G.fair.yMid,G.fair.zMid], 'paint', 0.30, 0, null, wear);
    quad(out, [fw,G.fair.yMid,G.fair.zMid],[-fw,G.fair.yMid,G.fair.zMid],[-fw,G.cabBackY+0.02,G.fair.zTop],[fw,G.cabBackY+0.02,G.fair.zTop], 'paint', 0.32, 0, null, wear);
    for(const sx of [-1,1]){
      const x=sx*fw;
      out.push(F(sx>0? [[x,G.fair.yFront,2.86],[x,G.cabBackY+0.02,2.86],[x,G.cabBackY+0.02,G.fair.zTop],[x,G.fair.yMid,G.fair.zMid]]
                     : [[x,G.cabBackY+0.02,2.86],[x,G.fair.yFront,2.86],[x,G.fair.yMid,G.fair.zMid],[x,G.cabBackY+0.02,G.fair.zTop]], 'paint', sx>0?0.12:-0.46));
    }
    wallY(out, G.cabBackY+0.02, -fw, fw, 2.86, G.fair.zTop, 'shade', -0.6, -1);
    // interior: high flat floor, dash, wheel, two buckets, bunk behind
    slab(out, [[-1.14,-0.84],[1.14,-0.84],[1.14,2.00],[-1.14,2.00]], G.cabFloorZ, 'rubber', -0.35, ribTex());
    boxAt(out, -1.10,1.10, 1.80, 2.00, 1.40, 1.60, 'dash', -0.4);
    tube(out, [-0.62,1.74,1.48],[-0.62,1.66,1.55], 0.18, 10, 'dash', -0.2);
    bar(out, [-0.62,1.78,1.30],[-0.62,1.68,1.48], 0.024, 'iron', -0.4);
    for(const sx of [-1,1]){
      boxAt(out, Math.min(sx*0.36,sx*0.88), Math.max(sx*0.36,sx*0.88), 1.08, 1.54, 1.36, 1.52, 'cloth', -0.45);
      boxAt(out, Math.min(sx*0.36,sx*0.88), Math.max(sx*0.36,sx*0.88), 0.96, 1.10, 1.52, 2.06, 'cloth', -0.55);
    }
    boxAt(out, -0.92,0.92, -0.78, 0.60, 1.30, 1.48, 'cloth', -0.5);            // bunk
    slab(out, [[-1.10,-0.80],[1.10,-0.80],[1.10,1.76],[-1.10,1.76]], 2.72, 'shade', -1.0);      // headliner
    buildDoors(out,s);
  }
  ...
  function buildWheels(out,s){
    ...
    bar(out,[0.12,1.30,0.50],[0.12,G.tandA+0.35,0.48],0.052,'iron',-0.3);
  ...
  function anchors(dir, opts){ opts=opts||{}; const s=resolve(opts), e=opts.elev;
    ...
    return {
      fifthWheel:P([0,G.fw.y,G.fw.topZ]), deck:P([0,-1.40,1.10]), gladHands:P([0,-0.87,1.98]),
      hoodLatch:P([0,2.07,1.60]),
      doorL:P([-G.hwCab,1.05,1.65]), doorR:P([G.hwCab,1.05,1.65]),
      roof:P([0,-0.40,G.fair.zTop]),
  ...
  root.AeroSemiIso = { W, H, PX, DIRS:8, pivot:{x:cx,y:groundY}, defaultElev:DEFAULT_ELEV,
