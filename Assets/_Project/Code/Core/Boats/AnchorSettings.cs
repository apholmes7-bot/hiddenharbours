...
{
    /// <summary>
    /// <b>Where a hull's ground tackle stands right now</b> — the one vocabulary the sim, the input
    /// and the helm's anchor switch all speak.
    ///
    /// <para><b>Why it lives in Core</b> (the <see cref="AnchorSettings"/> split, verbatim). The sim
    /// that moves this state is Boats-side (<c>BoatAnchor</c>), but the <b>dash switch</b> that shows
    /// it is <c>HiddenHarbours.UI</c>, which may not name a Boats type (rule 4). A second UI-side copy
    /// of the same three answers is the project's most-repeated rake — never compute one quantity two
    /// ways — so the enum moved here rather than being mirrored. Boats keeps <c>AnchorDrop</c>, which
    /// is the drop gate's private answer and crosses no module line.</para>
    /// </summary>
    ...
    /// Boats-side. The per-hull GROUND TACKLE — whether she carries a hook at all
    /// (<c>BoatHullDef.HasAnchor</c>), what it weighs (<c>AnchorMassKg</c>) and how much line comes
    /// with it (<c>RodeMeters</c>) — is hull data: bigger vessel, heavier hook, longer rode, deeper
    /// anchorages (P2).</para>
    ///
    ...
    {
        [Header("The ground tackle a hull carries when her Def does not say")]
        [Tooltip("The HOOK a dinghy-class hull carries (kg) — and, in the same breath, the mass the " +
                 "shared DragBrakeStrength below is quoted at. Two jobs, one number, on purpose: a " +
                 "hull whose BoatHullDef leaves AnchorMassKg at 0 carries exactly this anchor, and " +
                 "because the drag brake scales with mass/this, that hull's drag is byte-identical to " +
                 "what it was before hulls had anchor weights at all. Author a heavier hook on a hull " +
                 "and her dragging tackle checks her proportionally harder — twice the iron, twice " +
                 "the check (AnchorMath.DragBrakeStrengthFor).")]
        [Min(0.1f)] public float ReferenceAnchorMassKg;
        ...
        /// reference hook is a 6 kg dinghy grapnel — what the starting dory heaves over the bow, and
        /// the weight every other hull's tackle is measured against, so an un-authored hull drags
        /// exactly as she did before hulls carried anchor weights. The
