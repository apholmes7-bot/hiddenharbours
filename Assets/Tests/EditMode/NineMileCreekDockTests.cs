...
{
    /// <summary>
    /// Guards the VS-22 Nine Mile Creek arrival/dock geometry the NineMileCreekBuilder lays down (the owner-playtest
    /// "no way to disembark in Nine Mile Creek" fix). ControlSwitcher.InDockZone() is a pure DISTANCE test
    /// (Vector2.Distance(boat, dockZone) &lt;= radius) — the cove's proven pattern — so on a Cove→Nine Mile Creek
    ...
    ///
    /// The wharf was reoriented to face EAST so the crossing reads true (canon: Nine Mile Creek lies WEST of the
    ...
    /// report's note that the owner must re-run "Hidden Harbours ▸ Build Nine Mile Creek Scene" and re-test.
    /// </summary>
    public class NineMileCreekDockTests
    {
        ...
        public void Arrival_IsWithinDockRadius_OfTheDockZone()
        {
            ...
            Assert.LessOrEqual(d, NineMileCreekBuilder.DockZoneRadius,
                $"the Nine Mile Creek arrival ({NineMileCreekBuilder.ArrivalPos}) must park the boat within the dock " +
                $"radius ({NineMileCreekBuilder.DockZoneRadius} m) of the dock zone ({NineMileCreekBuilder.DockZonePos}) " +
        ...
        {
            // A real rig + a Nine Mile Creek anchor built from the SAME constants the NineMileCreekBuilder uses.
            ...
            hull.Id = "boat.NineMileCreek"; hull.CameraWorldHeightMeters = 14f; hull.DraughtMeters = 0.3f;
            ...
            // Board (the normal travel case is ARRIVING ABOARD), then arrive in Nine Mile Creek exactly as
            ...
            Assert.IsTrue(switcher.InDockZone(),
                "with the boat parked at arrival, the Nine Mile Creek dock zone is in range → E disembarks (the fix)");
        ...
        public void DisembarkPoint_IsOnTheWharfDeck_NotInOpenWater()
        {
            ...
            var p = NineMileCreekBuilder.DisembarkPos;
        ...
        public void Shoreline_DipsAroundTheWharfDeck_LeavingTheHeadDockable()
        {
            ...
            foreach (var p in pts) { if (p.x > headX) headX = p.x; if (Mathf.Approximately(p.x, NineMileCreekBuilder.DockZonePos.x)) reachesHead = true; }
            Assert.IsTrue(reachesHead, "the shoreline must trace out to the wharf head (x = dock zone x) so the boat docks against it");
            Assert.AreEqual(NineMileCreekBuilder.DockZonePos.x, headX, 1e-3f, "the head is the easternmost point of the fence (deep harbour open east of it)");
