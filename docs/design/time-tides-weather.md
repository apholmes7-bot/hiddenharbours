...
1. **Tide datum vs. art sea level.** We define tide as metres above chart datum, but the ¾ top-down camera shows water as a *plane*, not a side elevation. Need an agreed mapping from `tideHeight`/`waterDepth` → **visual cues** (shoreline waterline position, exposed-rock sprites toggling, flat tiles switching wet→dry, float heights). Coordinate with `art-and-audio-bible.md`. (Likely: per-region a small set of waterline states + tile wet/dry swaps keyed off `waterDepth` thresholds, rather than true vertical displacement.)
   - **Resolution shipped, then converged on the shader (St Peters, gameplay-systems).** The flat-tile-wet/dry-swap idea above first shipped as a runtime greybox overlay (`TidalFlatVisual`, `Code/Environment`): a 2 m grid of cells over the bar/flats whose colour was a pure function of the same `waterDepth = WaterLevel − terrainElevation` the walkability/boat-cross gate reads. It worked but, once the **layered water shader** (`HiddenHarbours/Water` + `WaterSurface`, **ADR 0010**) was live on St Peters reading the *same* deterministic tide + `ITidalTerrain` over the *same* flat, the 2 m grid sat **on top** of it and double-drew the shore as big flat blue/teal squares — the owner-reported blockiness, the smooth shader hidden underneath. **Per ADR 0012 ("converge the live shoreline on the shader path") the grid was RETIRED** (`TidalFlatVisual.cs` + its test deleted; the St Peters builder no longer stamps it). The shader now solely owns the reveal: its `clip(depth)` bares the authored sand ground when the tide recedes (`depth ≤ 0`) and renders a smooth depth-graded, foam-fringed water when covered — the same single height-map truth (§3.5), now smooth instead of a 2 m staircase. **P1 is untouched:** walkability + the boat-cross still read `WaterLevelAt`/`ITidalTerrain.ElevationAt` *directly* (not any visual), so the bar bares for clams and the crossing gates at the *same* tide levels; only the picture changed. The shader's own bake resolution was raised (96 → 192) so its wet edge is fine, not blocky (ADR 0012 §A).
