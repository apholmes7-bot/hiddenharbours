...
(function (root) {
  ...
  function buildDoors(out,s){for(const [sx,pose]of [[-1,s.dL],[1,s.dR]]){const a=sx*pose*65*DEG;part(out,T=>p2DoorLeaf(T,sx,G.doorY,G.hwCab,G.doorZ0,1.46,G.doorHead,s),p=>hingeZ(p,sx*0.98,G.doorY[1],Math.cos(a),Math.sin(a)));}}
  function p1Cab(out,s){
    ...
    p2Roof(out,G.hwCabRoof,G.cabBackY+.02,2.06,G.cabRoofZ);
    for(const sx of [-1,1]){
  ...
  function buildHoodClip(out,s){const a=-s.hood*G.hoodDeg*DEG;part(out,T=>p2Front(T,s,'conventional'),p=>rotX(p,G.hoodHinge.y,G.hoodHinge.z,Math.cos(a),Math.sin(a)));}
  ...
  const FIT={"belt":1.46,"rake":0.24,"front":2.42,"roofY":2.06,"roofZ":2.86,"drop":0.065,"xb":0.98,"xt":0.9,"hx":0.98,"hingeSetback":0.065};
  ...
  root.ConvBoxIso = { cabFit, W, H, PX, DIRS:8, pivot:{x:cx,y:groundY}, defaultElev:DEFAULT_ELEV,
