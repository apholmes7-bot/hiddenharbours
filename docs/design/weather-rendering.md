...
- **We take the *technique*, not the *palette*.** The reference reads as a **neon fantasy garden** —
  ...
  and *on-brand*. The signature target image is **a lit Greywick glowing warm through a grey fog at
...
- **Performance (rule 7) — the open cost question.** A full-screen graded pass + a cloud layer + a
  ...
  texture only where a scene needs many lights in fog (Greywick at dusk). Throttle uniform pushes to the
...
3. **Many-lights-in-fog.** Single-light globals (open water, one boat) vs a **light-accumulation texture**
   (Greywick at dusk, many windows/lamps). When does the texture earn its cost? (ADR 0016 already flags
   the extension.)
