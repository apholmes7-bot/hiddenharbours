...
(function (root) {
  ...
  /* ACCEPT THE RIG'S OWN VOCABULARY. This took a numeric 0–1 only, and clamp() passes a string
     straight through (`'low' < 0` and `'low' > 1` are both false) — so tideOf('low') returned
     waterM = NaN, every downstream comparison went false, and the plant came out at about 4% of its
     mass. Silently: the audit still said pass:true, legal:true, because a nearly empty sprite
     breaks no rule it measures. The rig publishes TIDE_KEYS; a caller using them is right, so they
     resolve here, and anything else that is not a finite number is a throw rather than a shrug. */
