// Sport Helm — diegetic control rig for the SPORT centre-console skiff (the console
// skiff's polished sister; companion to consoleRig.js, same geometry so the game
// picks it as an upgrade). UPGRADED hardware: a chrome destroyer wheel, chrome
// gauge bezels over WHITE dials (dark needles, sporty red counterweight), a brushed
// stainless binnacle carrying ONE single-lever control — push forward (up) for
// AHEAD, centre for NEUTRAL, pull aft (down) for ASTERN — that SWINGS on an arc
// about a chrome compass-rose hub, plus chrome switchgear and a chrome ignition
// barrel. No radar / MFD. Everything the game modifies is a parameter; the panel is
// static, moving parts are baked upright and rotated crisp at draw time.
(function (root) {
  const W = 600, TOPPAD = 40, H = 470 + TOPPAD;   // TOPPAD = headroom above the dash so a tall instrument can protrude
  const DEG = Math.PI / 180;

  // ---- palettes -------------------------------------------------------------
  const GRAPH  = ['#12171b','#1b2228','#28323a','#3a4750','#4f5e68','#6b7c86'];  // dark casings (switch panel)
  const STEEL  = ['#232a30','#39434b','#556069','#7a8892','#9db0b8'];            // screws
  const RUBBER = ['#0b0e11','#11161a','#1a2126','#252e35','#333f47'];            // black lever grip
  const CHROME = ['#1e242a','#3a454d','#647079','#9fb0b6','#cdd9db','#eff6f6'];  // polished stainless: reflection -> hotspot
  ...
  const TEAL   = '#7fd6c9';
  // white gauge dials
  const GFACE  = '#edefe8', GFACE2 = '#dbe0d8';
  const GTICK  = '#1b2427', GTICKD = '#5f7073';
  const GLABEL = '#1c7367';
  const NEEDLE = '#141a1d', NEEDLE_HL = '#39454a';
  const TICK   = GTICK, TICKDIM = GTICKD;
  const RED    = ['#7c2a20','#b83b2e','#e0554a'];
  const GREEN  = ['#1c6a3b','#2f9e57','#66d585'];
  const AMBER  = '#e6b53f';
  const LAMPOFF = '#16202a';
  const SPOT_ON = '#eef3ef', DECK_ON = TEAL;
  const BINTX  = '#123a36', BINTX_R = '#7c2a20';
  ...
  // ---- geometry (IDENTICAL to consoleRig so the game treats it as an upgrade)
  const CONSOLE = { x:14, y:44, w:572, h:396, r:20 };
  const WHEEL   = { cx:300, cy:256, r:112 };
  ...
  // ---- gauges: WHITE dials with chrome bezels -------------------------------
  function gaugeBezel(ctx, g){
    circle(ctx, g.cx, g.cy, g.r+7, CHROME[0]);
    ring(ctx, g.cx, g.cy, g.r+6, g.r+1, CHROME[2]);
    ctx.save(); ctx.beginPath(); ctx.rect(g.cx-g.r-7, g.cy-g.r-7, (g.r+7)*2, g.r+3); ctx.clip();
    ring(ctx, g.cx, g.cy, g.r+6, g.r+1, CHROME[5]); ctx.restore();
    ctx.save(); ctx.beginPath(); ctx.rect(g.cx-1, g.cy, g.r+9, g.r+9); ctx.clip();
    ring(ctx, g.cx, g.cy, g.r+5, g.r+2, CHROME[1]); ctx.restore();
    ring(ctx, g.cx, g.cy, g.r+6, g.r+5, CHROME[0]);
    circle(ctx, g.cx, g.cy, g.r, GFACE);
    ring(ctx, g.cx, g.cy, g.r, g.r-3, GFACE2);
    ring(ctx, g.cx, g.cy, g.r, g.r-1, '#c2ccc4');
    ctx.fillStyle = 'rgba(255,255,255,0.40)'; ctx.fillRect(g.cx-g.r+5, g.cy-g.r+6, g.r-2, 2);
  }
  ...
  function drawNeedle(ctx, g, px, py, tx, ty){
    ...
    thickLine(ctx, px, py, px-ux*tail, py-uy*tail, 3, RED[2]);           // sporty red counterweight
  ...
  function gaugeRPM(ctx, g, rpm){
    ...
    for (let i=0;i<=12;i++){ const v=i/12, deg=-135+270*v, major=(i%2===0);
      radialTick(ctx, g, deg, g.r-3, major?g.r-12:g.r-8, major?2:1, major?GTICK:GTICKD); }
    ...
    for (let n=0;n<=6;n++){ const v=n/6, d=dir((-135+270*v)*DEG), rr=g.r-22;
      textC(ctx, String(n), g.cx+d.x*rr, g.cy+d.y*rr-2, 1, n>=5?RED[1]:GTICK); }
    textC(ctx, 'X100', g.cx, g.cy-4, 1, GTICKD);
    textC(ctx, 'RPM', g.cx, g.cy+g.r-18, 2, GLABEL);
  ...
  function gaugeFUEL(ctx, g, fuel, low, blink){
    ...
    { const d=dir(fuelPhi(0)*DEG),   rr=g.r-15; textC(ctx, 'E', g.cx+d.x*rr, g.cy+d.y*rr-3, 2, RED[1]); }
    { const d=dir(fuelPhi(1)*DEG),   rr=g.r-15; textC(ctx, 'F', g.cx+d.x*rr, g.cy+d.y*rr-3, 2, GTICK); }
    { const d=dir(fuelPhi(0.5)*DEG), rr=g.r-16; ctx.fillStyle=GTICK; ctx.fillRect(g.cx+d.x*rr-1, g.cy+d.y*rr-2, 2, 4); }
    textC(ctx, 'FUEL', g.cx, g.cy+g.r-16, 2, GLABEL);
    circle(ctx, g.cx, py-14, 3, (low && blink) ? AMBER : '#c7d0c8');
    const d=dir(fuelPhi(fuel)*DEG);
    drawNeedle(ctx, g, g.cx, py, g.cx+d.x*rimR, g.cy+d.y*rimR);
  }

  // ---- gauge night backlight (warm amber over the white dials) --------------
  function gaugeNight(ctx, g){
    ...
    gr.addColorStop(0,'rgba(255,182,72,0.52)'); gr.addColorStop(0.65,'rgba(240,158,54,0.40)'); gr.addColorStop(1,'rgba(214,132,46,0.30)');
    ctx.fillStyle = gr; ctx.fillRect(g.cx-g.r, g.cy-g.r, g.r*2, g.r*2);
    ctx.restore();
    ctx.save(); ctx.globalCompositeOperation = 'lighter';
    const b = ctx.createRadialGradient(g.cx, g.cy, g.r*0.5, g.cx, g.cy, g.r+16);
    b.addColorStop(0,'rgba(255,184,74,0.20)'); b.addColorStop(1,'rgba(255,184,74,0)');
    ctx.fillStyle = b; ctx.fillRect(g.cx-g.r-18, g.cy-g.r-18, (g.r+18)*2, (g.r+18)*2);
    ctx.restore();
  }

  // ---- chrome destroyer wheel ----------------------------------------------
  function buildWheel(r){
    ...
    [90,210,330].forEach(deg=>{ const d=dir(deg*DEG);
      ...
      thickLine(g, cx+d.x*ro+1, cy+d.y*ro+1, cx+d.x*(ri-1)+1, cy+d.y*(ri-1)+1, 1, CHROME[1]); });
    circle(g, cx, cy, ro+3, CHROME[0]); circle(g, cx, cy, ro, CHROME[3]); circle(g, cx-2, cy-2, ro-2, CHROME[5]);
    circle(g, cx, cy, 4, CHROME[0]);
    const kd = dir(0), kx = cx+kd.x*(r-tw/2), ky = cy+kd.y*(r-tw/2);
    circle(g, kx, ky, kn, CHROME[0]); circle(g, kx, ky, kn-1, CHROME[3]); circle(g, kx-1, ky-2, kn-3, CHROME[5]);
  ...
  // ASTERN. The lever is its own little 3D rig that PIVOTS about this chrome
  // compass-rose hub — it swings on a real arc with true foreshortening and its
  // sprite frames bake straight out of it. Here we only draw the hub and
  // composite LeverRig.render(sig,'chrome') at the pivot.
  const LEVER = 'chrome';
  ...
  function driveHub(ctx){
    circle(ctx, DRIVE.px, DRIVE.pivotY, 22, CHROME[0]);
    circle(ctx, DRIVE.px, DRIVE.pivotY, 20, CHROME[2]);
    ctx.save(); ctx.beginPath(); ctx.rect(DRIVE.px-22, DRIVE.pivotY-22, 44, 22); ctx.clip();
    circle(ctx, DRIVE.px, DRIVE.pivotY, 20, CHROME[4]); ctx.restore();
    circle(ctx, DRIVE.px-1, DRIVE.pivotY-2, 16, CHROME[3]);
    ring(ctx, DRIVE.px, DRIVE.pivotY, 22, 20, CHROME[0]);
    circle(ctx, DRIVE.px, DRIVE.pivotY, 12, CHROME[1]);
    for (let k=0;k<8;k++){ const a=k*45*DEG, d=dir(a), long=(k%2===0), rO=long?11:6;
      thickLine(ctx, DRIVE.px, DRIVE.pivotY, DRIVE.px+d.x*rO, DRIVE.pivotY+d.y*rO, long?2:1, long?CHROME[5]:CHROME[3]); }
    circle(ctx, DRIVE.px, DRIVE.pivotY, 2, CHROME[0]);
  }
  ...
  function toggle(ctx, s, on, lampCol){
    ...
    rrect(ctx, cx-4, ty, 8, by-ty, 3, CHROME[1]);
    rrect(ctx, cx-4, up?ty:by-6, 8, 6, 3, CHROME[3]);
    ctx.fillStyle = CHROME[5]; ctx.fillRect(cx-2, up?ty+1:by-4, 3, 2);
    circle(ctx, cx, midY, 3, CHROME[5]);
  ...
  function buildKey(){
    ...
    thickLine(g, px, py, px, py-shaft, 4, CHROME[2]);
    ...
    circle(g, px, by+Math.round(bowH*0.5), 3, CHROME[2]);
    return { c, px, py };
  }
  function drawIgnition(ctx, s, running){
    circle(ctx, s.cx, s.cy, s.r, CHROME[0]);
    circle(ctx, s.cx, s.cy, s.r-1, CHROME[2]);
    ctx.save(); ctx.beginPath(); ctx.rect(s.cx-s.r, s.cy-s.r, s.r*2, s.r); ctx.clip();
    circle(ctx, s.cx, s.cy, s.r-1, CHROME[4]); ctx.restore();
    circle(ctx, s.cx-1, s.cy-2, s.r-4, CHROME[3]);
    ring(ctx, s.cx, s.cy, s.r, s.r-2, CHROME[0]);
  ...
  function paint(ctx, o){
    ...
    thickLine(ctx, 175, 27, 425, 27, 2, CHROME[3]);
    thickLine(ctx, 175, 47, 425, 47, 2, CHROME[1]);
    rrect(ctx, SPOTCAN.x, SPOTCAN.y, SPOTCAN.w, SPOTCAN.h, 5, CHROME[2]);
    rrect(ctx, SPOTCAN.x, SPOTCAN.y, SPOTCAN.w, 3, 2, CHROME[5]);
    circle(ctx, SPOTCAN.x+SPOTCAN.w-4, SPOTCAN.y+SPOTCAN.h/2, 6, CHROME[0]);
    ...
    // ---- console body (white, teal cove, stainless pin) ----
    ...
    ctx.fillStyle = CHROME[5]; ctx.fillRect(CONSOLE.x+8, CONSOLE.y+62, CONSOLE.w-16, 1);
    ctx.fillStyle = CHROME[2]; ctx.fillRect(CONSOLE.x+8, CONSOLE.y+63, CONSOLE.w-16, 1);
    ...
    // ---- switch panel (left, dark so chrome bats pop) ----
    ...
    // ---- binnacle casing (right, brushed stainless) ----
    rrect(ctx, BINN.x-2, BINN.y-2, BINN.w+4, BINN.h+4, BINN.r+1, CHROME[0]);
    rrect(ctx, BINN.x, BINN.y, BINN.w, BINN.h, BINN.r, CHROME[2]);
    rrect(ctx, BINN.x+2, BINN.y+2, BINN.w-4, 8, BINN.r-3, CHROME[5]);
    rrect(ctx, BINN.x+3, BINN.y+BINN.h-8, BINN.w-6, 5, 4, CHROME[0]);
    ctx.save(); ctx.beginPath(); ctx.rect(BINN.x+BINN.w-30, BINN.y, 34, BINN.h); ctx.clip();
    rrect(ctx, BINN.x, BINN.y, BINN.w, BINN.h, BINN.r, CHROME[1]); ctx.restore();
    ...
    textC(ctx, 'F', 570, 300, 1, BINTX);
    textC(ctx, 'N', 570, 318, 1, BINTX);
    textC(ctx, 'R', 570, 336, 1, BINTX_R);
    ...
    thickLine(ctx, WHEEL.cx, WHEEL.cy, WHEEL.cx, CONSOLE.y+CONSOLE.h-6, 18, CHROME[1]);
    thickLine(ctx, WHEEL.cx-5, WHEEL.cy+10, WHEEL.cx-5, CONSOLE.y+CONSOLE.h-6, 3, CHROME[4]);
    thickLine(ctx, WHEEL.cx+7, WHEEL.cy+10, WHEEL.cx+7, CONSOLE.y+CONSOLE.h-6, 2, CHROME[0]);
    circle(ctx, WHEEL.cx, WHEEL.cy, 20, CHROME[0]);
