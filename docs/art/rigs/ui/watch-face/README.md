...
Day is a positive grey-green panel; night lights the traditional green backlight. This is where time &
...
| `season` | string | season tag — pass `WatchRig.SEASON_ABBR[idx]` |
...
`SEASON_FULL` = `['Early Spring','High Summer','The Turn','Hard Winter']`. Button hit-boxes for the
preview live at `WatchRig.hit.light` and `WatchRig.hit.mode` (rig-local `{x,y,w,h}`).
