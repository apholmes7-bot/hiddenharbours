...
(function (root) {
  ...
  function buildDoors(out,s){for(const [sx,pose]of [[-1,s.dL],[1,s.dR]]){const a=sx*pose*65*DEG;part(out,T=>p2DoorLeaf(T,sx,G.doorY,G.hwCab,G.doorZ0,1.62,G.doorHead,s),p=>hingeZ(p,sx*1.18,G.doorY[1],Math.cos(a),Math.sin(a)));}}
  function p1Cab(out,s){
    ...
    p2Roof(out,G.hwCabRoof,G.cabBackY+.02,1.50,G.cabRoofZ);
    for(const sx of [-1,1]){
    ...
    // Broad, round shoulders carry the upper sleeper into the cab sides.
    const rows=[],xs=[-1,-.98,-.92,-.80,-.58,-.3,0,.3,.58,.80,.92,.98,1];
    for(let j=0;j<=18;j++){const t=j/18,y=G.cabBackY+.02+(1.50-G.cabBackY-.02)*t,e=t*t*t;
      const z=G.fair.zTop+(2.88-G.fair.zTop)*e;
      rows.push(xs.map(u=>[u*G.hwCabRoof,y,z-.27*Math.pow(Math.abs(u),5)*(1-.88*t)]));}
    artSurface(out,rows,'paint',.08);
    for(const sx of [-1,1])for(let j=0;j<rows.length-1;j++){
      const i=sx<0?0:xs.length-1,a=rows[j][i],b=rows[j+1][i];p2Face(out,[[sx*G.hwCabRoof,a[1],2.83],a,b,[sx*G.hwCabRoof,b[1],2.83]],'paint',-.03,[sx,0,0]);}
    p2Y(out,G.cabBackY+.01,rows[0].map(p=>[p[0],p[2]]).concat([[G.hwCabRoof,2.84],[-G.hwCabRoof,2.84]]),'paint',-.12,-1);
  ...
  const FOLD = { alloy:'chrome', p2Hood:'p2Pressed', p2Plate:'chrome', dash:'rubber' };
  ...
  function buildHoodClip(out,s){const a=-s.hood*G.hoodDeg*DEG;part(out,T=>p2Front(T,s,'aero'),p=>rotX(p,G.hoodHinge.y,G.hoodHinge.z,Math.cos(a),Math.sin(a)));}
  ...
  function buildFrame(out,s){p1Frame(out,s);p2RunningGear(out,s,false);}
  ...
  const FIT={"belt":1.62,"rake":0.23,"front":2.02,"roofY":1.5,"roofZ":2.88,"drop":0.0324,"xb":1.18,"xt":1.08,"hx":1.18,"hingeSetback":0.065};
  ...
  root.AeroSemiIso = { cabFit, W, H, PX, DIRS:8, pivot:{x:cx,y:groundY}, defaultElev:DEFAULT_ELEV,
