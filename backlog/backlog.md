...
| M2-39 ◐ | Diegetic interact highlight (shader, no UI) | art-pipeline + gameplay-systems | Facing-aware `IInteractable` candidate gets a subtle outline/rim shader; one bound input acts (pick up / place / grab) | **GAMEPLAY HALF DONE 2026-08-12** — `IInteractable` + `Interactables` registry + the pure `InteractResolver` (context → availability → reach → arc, then priority → distance → id) + `InteractVerb` dispatch, all in Core. **No new key**: the verb generalises E and is consulted *after* board/helm/step-ashore, so an empty registry is bit-for-bit the old behaviour. Exactly one candidate resolves, order-independently (all-permutations test). `WetBucketPoint` migrated off `F`. **STILL OPEN — art-pipeline:** the outline/rim shader itself; it rides the `InteractCandidateChanged` id and there is **no screen-space UI**, per (`deck-boarding-cleats-and-interact-capture` §3, `art/outline-interaction-language` §4.3) |
...
| M2-38 ✅ | Cleats, ropes & toss-a-line mooring | gameplay-systems + world-content | Per-hull named `CLEATS` (rig data) + shore cleats; grab a line and **toss it with the fishing-cast verb**; made fast = the boat holds | **DONE 2026-08-06.** Toss reuses the cast verb — the pure `FlickCastMath` moved to Core so rope and rod share ONE function. A made-fast line constrains tide/wind drift (sim keeps computing; the rope restrains the result). **The tide law:** scope must cover the 3-D cleat-to-cleat gap and only the shore end holds still, so a falling tide eats a short line's reach until the loop slips — cozy fail, coil and retry. Canon: `boats-and-navigation` §9.6 |
...
#### The realness pass (ADR 0027) — ten techniques taken from Unity's HDRP water, every one pixelized
...
> M3-23 (the wave field). M3-18/19 are Tier A and can start on owner ratification alone.
> **Re-ordered 2026-07-28** after the owner named wave variability the priority: **M3-24 (tuning, no code) runs
> first**, and **M3-23 (the spectrum) moves from last to second** — it is the keystone for variance in sizes,
> directions, and crests that build and collapse. M3-21 stays **externally clocked** against the in-flight
> dynamic-wake PRs, not phase-ordered. Order: **M3-24 → M3-18 → M3-23 → M3-26 → M3-19 → M3-20 → M3-22**, with
> M3-21 in parallel. **M3-25 (dispersion) rides with M3-18** — wavelength and speed must land together.
> ⚠️ The re-order brings the Tier B risk forward: M3-23's feel verdict now lands *before* absorption/reflections,
> and a bad verdict stalls the queue behind it. M3-24 exists partly to de-risk that — if the knobs deliver most of
> the ask, M3-23 can be scoped down or deferred again **on evidence**.
...
| M3-19 ✅ | Seabed absorption — `_SeabedTex` bake + per-channel Beer-Lambert (ADR 0027 P4) — **SHIPPED 2026-07-29** | art-pipeline + lead-architect | Bake the seabed over the `_HeightTex` world rect so the shader composites the bottom itself; `T = exp(−σ·2d)` per channel; `_AbsorptionBands` quantizes | Retires `_ShallowTranslucency`/`_ShallowMinAlpha` and **dissolves §17.3's cancellation by construction**; painted `_DepthRamp` untouched; σ = 0 exact passthrough; headless twins (monotone, red-before-blue, 2× path). Spec: `design/water-rendering.md` §17.7. Bake tool: `Hidden Harbours ▸ Dev ▸ Bake Seabed Texture`, 1.0 MB per region at 512². ⚠️ **Owner still owes the bake** — the feature is inert until a `_SeabedTex` is baked + assigned and `_Turbidity` is dialled above 0 |
| M3-20 ✅ | Object reflections — `HHReflect` renderer list, wave-warped (ADR 0027 P5) — **SHIPPED 2026-07-29** | lead-architect + art-pipeline | Fourth filtered list into `_HHReflectTex`; mirror about the ADR 0026 ground-contact pivot; water shader warps the lookup by `WaveFieldSample()` | **Revisits ADR 0010 addendum 8** on the `IsoFacetHullFeature` precedent; zero-cost-when-idle honoured; reuses shipped `WaterReflection` sea-state curves; point-filtered RT + lookup snapped on the **world** PPU grid (⚠️ screen-snapping crawls on every camera pan); night-lit sources ride the §11.6 post-grade compensation. Spec: `design/water-rendering.md` §26. ⚠️ **Nothing ships reflective** — add a `ReflectiveObject` to a prefab, then raise `_ObjectReflectStrength`. Fidelity probe RULED: a flat mirrored silhouette is enough at PPU 32, no interior mask or self-occlusion needed |
| M3-21 | ~~Advected foam buffer for wake~~ ✅ **SHIPPED 2026-08-01** (ADR 0027 #6) | art-pipeline | Ping-pong RT anchored to a **world** cell grid: advect along the shared wind/current drift, decay on a half-life, inject where hulls work against the water — **horizontally** (speed through the water) **and vertically** (heave rate relative to the wave surface). **Adds** to the existing foam; `BoatWakeEmitter` stays | The 2026-07-29 deferral was **overturned by the owner on 2026-08-01**: *"the hull doesn't create realistic foam when bobbing."* Bobbing churn is a one-boat defect the emitter cannot serve (it keys on speed). Ships **OFF** — `_WakeFoamStrength` 0 **and** no `FoamInjector` in any scene. Live spec: `docs/design/water-rendering.md` §28 |
| M3-22 | Wind fetch from the height map (ADR 0027 P6) | art-pipeline → gameplay-systems | March upwind across `_HeightTex` to a **fixed** step cap; lee shores calm, exposed shores build; visual first, then into the field | ⚠️ Fixed iteration count only — `[unroll]` over a runtime bound is a guarded magenta trap; promotion into the field earns a C# twin + ADR 0018 amendment |
| M3-23 | Spectrum-shaped swell — JONSWAP weighting, directional spreading, wave groups (ADR 0027 P2 ⬆ was P6) | lead-architect + gameplay-systems | Replace flat train weighting in `_WaveFieldParams` so the sea produces **sets**; explicitly **not** an FFT | ⚠️ **Changes what hulls ride** — C# twin, headless determinism tests, ADR 0018 amendment, and an owner **feel verdict** taken against the ADR 0023 hull set. Delivers *variance in sizes* (JONSWAP), *directions* (continuous spreading, not 3 discrete axes) and *crests that build and collapse* (grouping) |
| M3-24 | **Tuning pass — the sea's variability is set LOW ON PURPOSE (ADR 0027 P0, NO CODE)** | art-pipeline + owner | ① **View the sea in a STORM first** — `_OceanSwellStrength` already ramps 0.05 (fog) → **0.16 (storm = its design default)**, so the calm look may be the system working. ② Only then raise `_WindChop` 0.07/0.4, `_Octave3Weight` 0.108/0.3, `_FoamEvolveSpeed` 0.1/0.25 | ⚠️ **This REVISES the owner's own art direction, it is not a bug fix** — `_FbmStrength` sits at 0.035–0.08 in **all 8** mood materials independently. ⚠️ **Edit target differs per prop:** `_FbmStrength`/`_OceanSwell*` are **mood-eased** — edit `Art/Materials/WaterPresets/*.mat`, NOT `Water.mat` (tuning them there does nothing). ⛔ `_FbmScale` 3.26 is **RULED DELIBERATE** (was 3.27 pre-`f2574a4`) — leave it. ⚠️ Never tune `_Chop`/`_Roughness`/`_Flow` — sim-pushed |
