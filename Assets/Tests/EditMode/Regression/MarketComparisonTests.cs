...
{
    /// <summary>
    /// qa-test regression net (expand net 2) over the VS-16 second market — that Nine Mile Creek prices a
    ...
    /// (distinct from the float <c>PriceMultiplier</c> NineMileCreekMarketTests already covers).
    ///
    ...
    /// hold at Nine Mile Creek pays MORE than the cove — closing the gap this net originally flagged (the test
    ...
    public class MarketComparisonTests
    {
        ...
        // A config where Nine Mile Creek demands more than the cove (the production intent; values are the
        // test's own so the real GameConfig asset stays the single source of truth for shipped balance).
        private GameConfig MakeConfig(float cove = 1f, float nineMileCreek = 1.5f)
        {
        ...
        [Test]
        public void NextUnitPrice_IsDemandAware_NineMileCreekBeatsCove_ForTheSameGlut()
        {
            var config = MakeConfig(cove: 1f, nineMileCreek: 1.5f);
            ...
            nineMileCreek.RegisterSale(Cat, 5);
            Assert.AreEqual(cove.SupplyOf(Cat), nineMileCreek.SupplyOf(Cat), 1e-4f, "identical glut by construction");

            int covePrice = cove.NextUnitPrice(Cat, baseValue: 30, elasticity: 0.4f);
            int NineMileCreekPrice = nineMileCreek.NextUnitPrice(Cat, baseValue: 30, elasticity: 0.4f);

            Assert.Greater(NineMileCreekPrice, covePrice,
                "for the same glut, Nine Mile Creek's higher demand quotes the next unit higher than the cove");
        }

        [Test]
        public void NextUnitPrice_EqualDemand_QuotesTheSame_IsolatingDemandAsTheLever()
        {
            var config = MakeConfig(cove: 1.2f, nineMileCreek: 1.2f); // same D at both
            var cove = MakeMarket(config, MarketId.Cove);
            var nineMileCreek = MakeMarket(config, MarketId.NineMileCreek);
            cove.RegisterSale(Cat, 5);
            nineMileCreek.RegisterSale(Cat, 5);

            Assert.AreEqual(cove.NextUnitPrice(Cat, 30, 0.4f), nineMileCreek.NextUnitPrice(Cat, 30, 0.4f),
        ...
        [Test]
        public void SellScreenPath_IsDemandAware_NineMileCreekPaysMoreThanCove()
        {
            var config = MakeConfig(cove: 1f, nineMileCreek: 1.5f);
            var cove = MakeMarket(config, MarketId.Cove);
            var nineMileCreek = MakeMarket(config, MarketId.NineMileCreek);
            ...
            int NineMileCreekPaid = SellService.SellAll(HoldOf(5, 24, 0.4f), new FakeWallet(), nineMileCreek);

            // Market depth 2 threaded per-category demand through SellPricing/SellService, so the screen's
            // payout (and the instant FishBuyer.SellAll that now routes through it) honours Nine Mile Creek's
            // higher demand: selling the same hold at Nine Mile Creek pays MORE. This was the documented gap this
            // net flagged — the characterization here was flipped from AreEqual once the gap was closed.
            Assert.Greater(covePaid, 0);
            Assert.Greater(NineMileCreekPaid, covePaid,
                "selling via the VS-18 screen now honours market demand — Nine Mile Creek pays more than the cove");
        }
    }
}
