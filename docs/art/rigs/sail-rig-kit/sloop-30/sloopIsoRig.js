/* Hidden Harbours — parametric ISO SLOOP 30 (fibreglass fractional sloop) — M2 bake recipe, ADR-0006,
   same pipeline as sportSkiffIsoRig.js / lobsterBoatIsoRig.js. PASS 3 COPY (main 70969b68, preserves S0): refined deck, cockpit, accommodation and pose;
   ...
   whole picture from one law, sailPose():
     ...
     heel  = min(24, 0.067*aws^2*k(awa)*drive) degrees to leeward, on top of rock(i)
...
(function (root) {
  ...
  const GLAS  = ['#101c27','#1b2d3c','#2b4354','#486477','#78929e'];                      // smoked blue-grey glass
  ...
  // ---- plan constants ----------------------------------------------------------------------------
  const RF = { yA:-0.55, yF:3.05, yNose:3.45, hA:0.62, hF:0.42, sdA:0.42, sdF:0.54, tumble:0.12 };   // coachroof
  ...
  const CK = { yA:-0.62, yB:-3.05, yT:-4.55, xi:0.48, seatZ:1.60, coamZ:1.88, coamT:0.10 };   // cockpit
  ...
  const HATCH = { x0:-0.44, x1:0.44, y0:-0.60, y1:0.08, travel:0.52, aperture:{ x0:-0.34, x1:0.34, y0:-0.55, y1:-0.08 } };
  const RIG = { mast:MAST, goose:GOOSE, boomL:BOOM_L, main:MAIN, forestay:FORESTAY, jib:JIB, spreaders:SPREADERS, chain:CHAIN };

  const FIT={bulkheadY:2.40, setteeA:0.90, tableY:1.65, tableHalfY:0.50};
  ...
  (function build(){
    ...
    (function(){
      ...
      for(let i=0;i<NH;i++){
        ...
        face(F,[[-w0,y0,SOLE],[w0,y0,SOLE],[w1,y1,SOLE],[-w1,y1,SOLE]].reverse(),'teak',(i%2?0.05:-0.30));
      }
      ...
      for(const s of [-1,1]){
        ...
        for(let i=0;i<NS2;i++){
          ...
          face(F,s>0?top.slice().reverse():top,'teak',(i%2?0.15:-0.20),0.01);
          ...
          face(F,s>0?ct.slice().reverse():ct,'paint',0.6,0.01);
        ...
        for(const [c0,c1] of [[-0.75,-1.75],[-1.95,-2.90]]){
          const outer=Math.min(xCoam(c0),xCoam(c1))-CK.coamT;
          const xm=s*(xw+outer)/2, hw=(outer-xw)/2-0.025;
          cushion(F,[xm,(c0+c1)/2,CK.seatZ+0.045],[hw,(c0-c1)/2-0.025,0.045],'uph',0.30,0.012);
          cushion(F,[s*(outer-0.03),(c0+c1)/2,CK.seatZ+0.21],[0.045,(c0-c1)/2-0.055,0.10],'uph',0.05,0.012);
        }
      }
    ...
    (function(){
      ...
      const ys=[...new Set(Array.from({length:N+1},(_,i)=>yA+(yF-yA)*i/N).concat([HATCH.aperture.y1]))].sort((a,b)=>a-b);
      for(let i=0;i<ys.length-1;i++){
        ...
        if(y0>=A.y0-1e-6 && y1<=A.y1+1e-6){
          for(const side of [-1,1]){
            const q=[[side*A.x1,y0,z0],[side*(h0-RF.tumble),y0,z0],[side*(h1-RF.tumble),y1,z1],[side*A.x1,y1,z1]];
            face(F,side>0?q:q.reverse(),'paint',0.55,0,{lid:1});
            face(F,[[side*A.x1,y0,z0-0.10],[side*A.x1,y1,z1-0.10],[side*A.x1,y1,z1],[side*A.x1,y0,z0]],'teak',0,0,{lid:1});
          }
        }else face(F,[[-(h0-RF.tumble),y0,z0],[h0-RF.tumble,y0,z0],[h1-RF.tumble,y1,z1],[-(h1-RF.tumble),y1,z1]],'paint',0.55,0,{lid:1});
        if(y0>HATCH.aperture.y1 && y1<2.95){
        ...
        for(const s of [-1,1]){
          const q=[[s*h0,y0,d0],[s*h1,y1,d1],[s*(h1-RF.tumble),y1,z1],[s*(h0-RF.tumble),y0,z0]];
      ...
      for(const s of [-1,1]) for(const [w0,w1] of [[0.05,1.25],[1.55,2.75]]){
        ...
        for(let i=0;i<NW;i++){
          ...
          const zb0=deckZ(uOf(y0))+0.16+0.07*rfT(y0), zb1=deckZ(uOf(y1))+0.16+0.07*rfT(y1), zt0=roofZ(y0)-0.10, zt1=roofZ(y1)-0.10;
          const xw=(y,z)=>{ const t=(z-deckZ(uOf(y)))/(roofZ(y)-deckZ(uOf(y))); return hxRoof(y)-RF.tumble*t+0.012; };
      ...
      const hF=hxRoof(yF)-RF.tumble, zF=roofZ(yF), yN=RF.yNose, zN=deckZ(uOf(yN)), hN=hxRoof(yN)*0.92;
      face(F,[[-hF,yF,zF],[hF,yF,zF],[hN,yN,zN],[-hN,yN,zN]],'paint',0.5,0,{cut:'cabin',cutN:[0,1,0]});
      const windscreen=(x,t)=>[x,lerp(yF,yN,t),lerp(zF,zN,t)+0.006];
      face(F,[windscreen(-0.34,0.18),windscreen(0.34,0.18),windscreen(0.27,0.74),windscreen(-0.27,0.74)],'glas',0.1,0.015,{cut:'cabin',cutN:[0,1,0]});
      ...
      for(const [hy,hs] of [[1.55,0.28],[2.55,0.24]]){
        const hp=(x,y,z)=>[x,y,roofZ(y)+z];
        face(F,[hp(-hs-0.03,hy-hs-0.03,0.03),hp(hs+0.03,hy-hs-0.03,0.03),hp(hs+0.03,hy+hs+0.03,0.03),hp(-hs-0.03,hy+hs+0.03,0.03)],'steel',0.3,0.01,{lid:1});
        face(F,[hp(-hs,hy-hs,0.04),hp(hs,hy-hs,0.04),hp(hs,hy+hs,0.04),hp(-hs,hy+hs,0.04)],'glas',0.1,0.015,{lid:1});
      }
      for(const side of [-1,1]){
        const rail=y=>[side*(hxRoof(y)-0.22),y,roofZ(y)+0.11];
        for(const [a,b] of [[0.70,1.70],[1.90,2.75]]){
          bar(F,rail(a),rail(b),0.023,'teak',0.3,0.015,{lid:1});
          for(const y of [a,b]) bar(F,[rail(y)[0],y,roofZ(y)+0.01],rail(y),0.025,'steel',0.2,0.01,{lid:1});
        }
      }
    ...
    (function(){
      ...
      const NS3=8, y0s=-0.05, y1s=FIT.bulkheadY;
      for(let i=0;i<NS3;i++){
        const y0=y0s+(y1s-y0s)*i/NS3, y1=y0s+(y1s-y0s)*(i+1)/NS3;
        const w0=half(y0,CAB_SOLE), w1=half(y1,CAB_SOLE);
      ...
      box(F,[-0.78,0.40,(CAB_SOLE+1.25)/2],[0.32,0.45,(1.25-CAB_SOLE)/2],'teak',-0.1,0,null,C);
      box(F,[-0.78,0.14,1.265],[0.17,0.14,0.012],'steel',0.5,-0.02,null,C);
      box(F,[-0.76,0.64,1.28],[0.20,0.20,0.03],'blk',0.2,-0.02,null,C);
      for(const [bx,by] of [[-0.86,0.56],[-0.66,0.56],[-0.86,0.74],[-0.66,0.74]]) prism(F,[bx,by,1.31],0.045,0.012,8,'steel',0.7,-0.03,C);
      // head compartment, starboard aft: a wall box with a teak door (culled when it faces the camera)
      const HD={x0:0.45,x1:1.08,y0:-0.05,y1:0.85,z1:2.05};
      ...
      face(F,[[HD.x0-0.012,HD.y0+0.10,CAB_SOLE+0.01],[HD.x0-0.012,HD.y0+0.72,CAB_SOLE+0.01],[HD.x0-0.012,HD.y0+0.72,1.85],[HD.x0-0.012,HD.y0+0.10,1.85]],'teak',0.2,0.02,Object.assign({cutN:[-1,0,0]},CC));
      ...
      box(F,[0.80,0.44,CAB_SOLE+0.20],[0.18,0.22,0.20],'paint',0.3,0,null,C);   // the toilet
      // settees with cushions, backrests against the hull, lockers above (INTERACT locker = starboard)
      for(const s of [-1,1]){
        const y0=FIT.setteeA, y1=FIT.bulkheadY-0.05, N2=4;
        for(let i=0;i<N2;i++){
          ...
          const lk=[[s*(la-0.30),ya,1.30],[s*(lb-0.30),yb,1.30],[s*(lb-0.30),yb,1.72],[s*(la-0.30),ya,1.72]];
      ...
      box(F,[0,FIT.tableY,1.04],[0.28,FIT.tableHalfY,0.02],'teak',0.5,-0.01,null,C);
      bar(F,[0,FIT.tableY,CAB_SOLE],[0,FIT.tableY,1.02],0.03,'steel',0.4,-0.1,C);
      ...
      const yb2=FIT.bulkheadY, hb=half(yb2,1.2)+0.05, zt=roofZ(yb2)-0.04;
      ...
      box(F,[0.28,2.83,CAB_SOLE+0.55],[0.20,0.26,0.05],'cream',0.8,0.01,null,C);   // pillows
      box(F,[-0.28,2.83,CAB_SOLE+0.55],[0.20,0.26,0.05],'cream',0.8,0.01,null,C);
    })();
    for(const f of F) if(f.inside && !f.lip){
      f.v=f.v.map(([x,y,z])=>{
        const limit=Math.max(0.02,halfAtZ(uOf(y),Math.min(z,deckZ(uOf(y))),1)-0.012);
        return [Math.max(-limit,Math.min(limit,x)),y,z];
      });
    }
  ...
  function sailPose(o){
    ...
    const drive=(hoist>0.02?0.65*hoist*(0.25+0.75*fillM):0)+(furl<0.97?0.35*(1-furl)*(0.25+0.75*fillJ):0);
    const heelDeg = Math.min(24, 0.067*aws*aws*k*drive)*(irons?0.15:1);
  ...
  function dynamicFaces(o, pose, view){
    ...
    if(showSails){
      ...
      const bag=[];
      cushion(bag,[G[0],G[1]-BOOM_L*0.52,G[2]+0.085+bh],[bw,BOOM_L*0.46,bh],'canvas',0.1,0.01,RX);
      bag[0].mat=topMat;
      for(const f of bag){f.v=f.v.map(xfB);D.push(f);}
    ...
    if(view!=='cabin'){
      ...
      const xfH=p=>[p[0],p[1],p[2]+roofZ(p[1])-roofZ(yc)];
      if(ht>0.05){
        const A=HATCH.aperture;
      ...
      box(D,[0,yc,zc],[HATCH.x1,(HATCH.y1-HATCH.y0)/2,0.03],'paint',0.5,0.005,xfH);
      face(D,[[-0.30,yc-0.28,zc+0.031],[0.30,yc-0.28,zc+0.031],[0.30,yc+0.28,zc+0.031],[-0.30,yc+0.28,zc+0.031]].map(xfH),'glas',0.1,0.015);
    }
    // ---- wheel ----
  ...
  // Bounds before raster clipping; same cutaway visibility rules as the painter.
  function bounds(dir, opts){
    const r=allFaces(dir,opts), B=camBasis(r.o), view=r.o.view;
    let x0=Infinity,y0=Infinity,x1=-Infinity,y1=-Infinity;
    for(const f of r.faces){
      if(f.inside && view!=='cabin') continue;
      if(f.lid && view==='cabin') continue;
      if(f.under && !r.o.underbody) continue;
      if(view==='cabin'){
        if(f.cut && f.cutN && facingHull(f.cutN,B)) continue;
        if(f.lip && !facingHull(f.cutN,B)) continue;
      } else if(f.lip) continue;
      const vs=f.v.map(v=>projVert(...v,B));
      if(view==='cabin' && f.cut && !f.cutN && facingN(normal(vs[0],vs[1],vs[2]),B)) continue;
      for(const p of vs){x0=Math.min(x0,p.sx);x1=Math.max(x1,p.sx);y0=Math.min(y0,p.sy);y1=Math.max(y1,p.sy);}
    }
  ...
  function gameplayGeometry(){
    ...
    const cockpit={ id:'cockpit_sole', z:SOLE, winding:'ccw_from_above', polygon:poly.map(p=>[r3(p[0]),r3(p[1])]),
      note:'Teak-planked self-draining sole: a 0.96 m walkway between the benches from the companionway bulkhead (y='+CK.yA+') to the bench ends at y='+CK.yB+', then the full-width helm area to the open transom at y='+r3(-L/2)+'. The walk-through in the transom (x ±0.38) steps 0.20 m down onto the swim step.',
      _notes:[
        ...
        { id:'bench_port', kind:'obstruction', type:'seat', footprint:{x:[-r3(xCoam(-1.8)-CK.coamT),-CK.xi],y:[CK.yB,CK.yA]}, height_above_floor_m:{top:r3(CK.seatZ-SOLE)}, treatment:'step_over', provenance:'exact, CK const; cushions +0.09 in two pads; backrests +0.31; published as DECK cockpit_seat_port as well — you stand on it to reach the boom' },
    ...
    const seat=(s)=>({ id:'cockpit_seat_'+(s<0?'port':'stbd'), z:CK.seatZ, winding:'ccw_from_above',
      ...
      _notes:[{ id:'coaming', kind:'obstruction', type:'bulkhead_step', footprint:{x:'0.10 m band outboard of the seat', y:[CK.yB,CK.yA]}, height_above_floor_m:{top:r3(CK.coamZ-CK.seatZ)}, treatment:'step_over', provenance:'exact, CK.coamZ/coamT — the route from the seat up onto the side deck' },
              { id:'cushions', kind:'surface', type:'cushion', footprint:{y:[[-1.75,-0.75],[-2.90,-1.95]]}, height_above_floor_m:{top:0.09,backrest:0.31}, treatment:'flat', provenance:'exact, cockpit build(); backrests along outboard cushion edge' }] });
    ...
    const coachroof={ id:'coachroof', winding:'ccw_from_above', polygon3d:roofPoly,
      ...
      _notes:[
        ...
        { id:'deck_hatches', kind:'surface', type:'hatch_lid', footprint:{y:[[1.27,1.83],[2.31,2.79]]}, height_above_floor_m:{top:0.04}, treatment:'flat', provenance:'exact, coachroof build()' },
    ...
    for(const side of [1,-1]) for(let i=0;i<=8;i++){
      const t=side>0?i/8:1-i/8, y=lerp(-0.05,FIT.bulkheadY-0.02,t);
      cabinPoly.push([r3(side*(halfAtZ(uOf(y),CAB_SOLE,1)-0.05)),r3(y)]);
    }
    const cabin={ id:'cabin_sole', z:CAB_SOLE, level:'cabin', winding:'ccw_from_above', polygon:cabinPoly,
      ...
      _notes:[
        { id:'galley', kind:'obstruction', type:'counter', footprint:{x:[-1.10,-0.46],y:[-0.05,0.85]}, height_above_floor_m:{top:0.90}, treatment:'waist_block', provenance:'exact, cabin build(); stove on top at y 0.44..0.84 (INTERACT stove)' },
        { id:'head', kind:'obstruction', type:'compartment', footprint:{x:[0.45,1.08],y:[-0.05,0.85]}, height_above_floor_m:{top:1.70}, treatment:'wall', provenance:'exact, HD const; teak door on the inboard face' },
        ...
        { id:'settees', kind:'obstruction', type:'seat', footprint:{x:'outboard of ±0.55 to the hull',y:[FIT.setteeA,FIT.bulkheadY-0.05]}, height_above_floor_m:{top:0.40}, treatment:'step_over', provenance:'exact, cabin build(); lockers above from 0.95 to 1.37 (INTERACT locker = starboard)' },
        { id:'table', kind:'obstruction', type:'table', footprint:{x:[-0.28,0.28],y:[FIT.tableY-FIT.tableHalfY,FIT.tableY+FIT.tableHalfY]}, height_above_floor_m:{top:0.71}, treatment:'waist_block', provenance:'exact, cabin build()' },
    ...
    const INTERACT=[
      ...
      { id:'jib_winch_port', action:'trim_jib', label:'Port primary', verb:'Trim the jib', level:'cockpit_seat_port', pos:P3(A.winchPort), reach_point:[-0.70,-2.10,CK.seatZ], visible_facings:['N','NE','E','SE','S','SW','W','NW'], mech:'opts.jib 0..1; opts.grind:"jib" animates the handle on the LEEWARD primary', provenance:'WINCH.primary*' },
      { id:'jib_winch_stbd', action:'trim_jib', label:'Starboard primary', verb:'Trim the jib', level:'cockpit_seat_stbd', pos:P3(A.winchStbd), reach_point:[0.70,-2.10,CK.seatZ], visible_facings:['N','NE','E','SE','S','SW','W','NW'], mech:'as port', provenance:'WINCH.primary*' },
      ...
      { id:'furler', action:'furl_jib', label:'Furling line', verb:'Furl / unfurl the jib', level:'cockpit_sole', pos:P3(A.furlerCleat), reach_point:[-0.38,-1.05,SOLE], visible_facings:['N','NE','E','SE','S','SW','W','NW'], mech:'opts.furl 0..1 — the line runs the port deck to the drum', provenance:'furling line build()' },
      { id:'stove', action:'cook', label:'Stove', verb:'Cook', level:'cabin_sole', pos:[-0.76,0.64,1.28], reach_point:[-0.20,0.64,CAB_SOLE], visible_facings:facings(-1,0), mech:'galley, port aft', provenance:'cabin build()' },
      ...
      { id:'bunk', action:'sleep', label:'V-berth', verb:'Sleep · save', level:'cabin_sole', pos:[0,3.70,r3(CAB_SOLE+0.50)], reach_point:[0,FIT.bulkheadY-0.15,CAB_SOLE], visible_facings:facings(0,1), mech:'InteriorBed (rest + save) — 2.00 m V-berth through the forward bulkhead door', provenance:'cabin build()' },
    ];
    const SAIL={ rig:'fractional sloop · deck-stepped mast on the coachroof · two swept spreader sets · split backstay · roller-furling jib',
      ...
      main:{ P_m:MAIN.P, E_m:MAIN.E, head_m:MAIN.head, roach_m:MAIN.roach, battens:MAIN.battens, area_m2:r3(0.5*MAIN.P*MAIN.E*1.18),
        hoist:'opts.hoist 0..1 — luff = P*hoist up the mast; the stack pack on the boom grows 0.12 -> 0.40 m as the cloth comes down', stored:'hoist 0 + cover:true — the stack pack zipped; the STORED state', sheet:'opts.main 0..1 — 1 hardened (boom 4°), 0 eased (boom 86°)', luff_on:'mast aft face', foot_on:'boom (loose-footed)' },
      jib:{ luff_m:r3(JIB.luff), LP_m:JIB.LP, area_m2:r3(0.5*JIB.luff*JIB.LP), overlap:'105% of J', tack:P3({x:JIB.tack[0],y:JIB.tack[1],z:JIB.tack[2]}), head:P3({x:JIB.head[0],y:JIB.head[1],z:JIB.head[2]}),
        furl:'opts.furl 0..1 — rolled onto the forestay; rolled radius 0.035+0.07*furl, the UV strip is the canvas slot', sheet:'opts.jib 0..1 — 1 hardened (9°), 0 eased (85°)', leads:{ cars:[[-r3(carX()),CAR_Y],[r3(carX()),CAR_Y]], primaries:[[-r3(WINCH.primaryX()),WINCH.primaryY],[r3(WINCH.primaryX()),WINCH.primaryY]] } },
      pose_law:{ inputs:'awa (deg, + = wind over the starboard bow), aws (kn), main 0..1, jib 0..1, frame 0..7',
        ...
        stall:'aoa>40 — full but dead; camber eases 40% by aoa 80', heel_deg:'min(24, 0.067*aws^2*k(awa)*drive) to leeward; k=1 to 100°, 0.35 dead downwind; 15% in irons; drive = 0.65*deployed_main*(0.25+0.75*fill_main) + 0.35*deployed_jib*(0.25+0.75*fill_jib); deployed_main = hoist if >0.02 else 0; deployed_jib = 1-furl if furl<0.97 else 0',
    ...
    return {
      ...
      _excluded:{ LADDER:'None — a 30-footer: the coachroof is a step up from the bench, the swim step a step down from the sole.',
        sprayhood_bimini:'Not modelled in this copy (clutter at 32 px/m); the canvas slot dresses the stack pack and the jib UV strip.',
  ...
  root.SloopIso = { W, H, PX, DIRS:8, pivot:{x:cx,y:cy}, defaultElev:DEFAULT_ELEV,
    ...
    render, bounds, renderWheel, wheelFaces, WHEEL_GEO, WHEEL_HUB, WHEEL_LOCK, ROCK, rock:rockMotion,
    ...
    FIT, SCHEMES, schemeIds:Object.keys(SCHEMES), defaultScheme:DEFAULT_SCHEME, SLOTS, palette, rampFrom, chipWall, C_CAP };
