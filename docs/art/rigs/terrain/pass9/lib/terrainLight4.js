/* Hidden Harbours — TERRAIN LIGHT 4.  The floor on the trees' light contract.

   Pass 4 took the light out of the trees and left the ground behind: WeatherSky.lightTile lit the
   pixel terrain with N·L quantised to five steps, a flat sky term, a noise mask for snow and a 15 %
   darkening for rain, and every mark kept the upper-left tip and down-right seam it was painted with.
   So in the morning a pebble was lit from the west while its tree threw a shadow to the west; under
   overcast (no sun, no N·L) the floor went flat; snow ignored the marks; rain darkened sand and granite
   alike and left no water anywhere. What this module does, per texel, with TreeRig4.relight's law:

     A. SKY × VISIBILITY.  kS·skyI·(0.30 + 0.70·AO)·(0.45 + 0.55·up). AO is a horizon test on the
        height field in 8 directions, so a seam, a hoofprint or the foot of a cobble keeps its shade
        with no sun at all. Overcast is no longer flat.
     B. SUN × SELF-SHADOW.  kD·sunI·(N·L)^1.25, times a march along the sun through the height field:
        at golden hour a tussock throws ten texels of shadow, at noon one. Sprite shadows arrive as
        the trees' levels (1 canopy · 2 partial · 3 full).
     C. LIVE TIP AND SEAM.  Per MARK (PxLang tile.mark), recomputed for this sun: the mark's extreme
        texel toward the sun a band up when lit, its edge on the far side a band down where it stands
        proud. Materials authored before this pass have their upper-left tip and seam taken out first
        (un-baked), so no mark is lit twice.
     D. ONE BAND LADDER.  Absolute light (not normalised per sky) mapped to one band offset against
        the authored band, calibrated so the 14:00 key leaves flat ground where it was painted —
        which is what the trees do: dawn, dusk and moonlight darken the ground and the tree together.
     E. SURFACE CLASS.  veg · soil · sand · rock · mud · water · weed · shell · litter. Rain darkens
        by porosity (wet sand 30 %, granite 12 %, turf barely), puts glints on the lit tips of glossy
        classes (rock, weed, shell, mud) and a sky sheen on wet mud and weed; hollows fill to
        PUDDLES that take the sky and ring with the rain.
     F. SNOW BY SHAPE.  Cover arrives in hollows and on the flat first, proud tips poke through,
        slopes and canopy shade hold less, water never snows; the snow is banded by the same light
        (blue in shade) and sparkles in sun, the ground at a patch edge goes damp.
     G. WATER AND THE SWASH.  With an elevation field and a tide level: wave normals animated on the
        16-frame loop and scaled by wind, the sky's colour on the crests, sun glints, whitecaps in a
        gale, rain rings; shallows show the bottom through three steps of water; a swash front runs
        up and back with foam on its edge and a glossy wet band behind it; everything below the last
        high water is damp.
     H. FOG AND GRADE.  The trees' fog steps and the trees' grade, verbatim, so a tree and the ground
        under it land on one palette for any sky.

     TerrainLight4.gbuf(tile, o)      -> G   o = {relief, wrap, live[], cls[], elev, far(y)}
     TerrainLight4.relight(G, sky, o) -> RGBA  o = {x0, y0, w, h, lv, frame, tide, tideHigh, out, only}
     TerrainLight4.view(G, ch, sky, o) channels: lit unlit normal ao sun height class wet snow marks
     TerrainLight4.dynamic(G, sky, o)  region-local indices that animate (water, swash, puddle rings)
   Runs in the bake sandbox and the browser; needs PxLang.                                            */
(function (root) {
  'use strict';
  const P = root.PxLang; if (!P) throw new Error('terrainLight4: pixelLanguage.js must load first');
  const { hash2 } = P;
  ...
  function gbuf(t, o) {
    ...
    return { t, n, m, N, wrap, rel, ix, H, hmax, nx, ny, nz, ao, proud, hollow, pond, cls, pc, band0, elev: o.elev || null, far: o.far || null, water: o.water || null, fetch: o.fetch || null };
  }
  ...
  function relight(G, sky, o) {
    ...
    const lv = o.lv || null, tide = o.tide, tideHigh = o.tideHigh == null ? (tide == null ? null : tide + 0.35) : o.tideHigh, elev = G.elev;
    ...
    for (let q = 0; q < NI; q++) {
      ...
      const vis = sunOn && nl > 0 && lvl < 3 ? sunVis(G, X, Y, L) * (lvl === 2 ? 0.45 : 1) : 0;
      const sun = kD * sunI * (nl > 0 ? Math.pow(nl, 1.25) : 0) * vis;
      const E = (kS * skyI * (0.30 + 0.70 * ao) * (0.45 + 0.55 * nZ) * (lvl >= 1 ? 0.78 : 1) + sun) * expo;
      ...
      if (elev && tide != null) {
        ...
        if (e < front) { isW = true; depth = front - e; if (depth < 0.018 && hash2(X >> 1, 3, 5 + fi) < 0.6) foam = 1; }
        else {
      ...
      if (isW) {
        ...
        const q1 = Y * 0.62 + 1.6 * Math.sin(X * 0.043 + Y * 0.017) + 3.1 * vn(X / 58, Y / 21, 13) - ph;
        ...
        if (foam) { pid = PF; bi = hash2(X, Y, 29 + fi) < 0.6 ? 3 : 4; spc = 0; }
  ...
  function view(G, ch, sky, o) {
    ...
    for (let y = 0; y < Hh; y++) for (let x = 0; x < W; x++) {
      ...
      else if (ch === 'ao') r = g = b = G.ao[i] * 255;
      else if (ch === 'height') r = g = b = (G.H[i] - h0) / Math.max(1e-3, h1 - h0) * 255;
      else if (ch === 'sun') { const lvl = o.lv ? o.lv[y * W + x] : 0, nl = G.nx[i] * L[0] + G.ny[i] * L[1] + G.nz[i] * L[2], v = (sky.sunI > 0.01 && nl > 0 && lvl < 3 ? sunVis(G, X, Y, L) * (lvl === 2 ? 0.45 : 1) : 0) * Math.max(0, nl); r = 24 + v * 231; g = 22 + v * 200; b = 30 + v * 120 + (lvl === 1 ? 40 : 0); }
  ...
  root.TerrainLight4 = { CL, CLASSES, CLASS_COL, CLASS_OF, classify, SNOWR, FOAMR, DEEP, SHOAL, LOOP, UNLIT, VIEW, gbuf, relight, dynamic, view, swashAt, sunVis, bandOff, vn };
