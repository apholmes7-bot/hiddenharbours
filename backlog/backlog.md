...
| M3-24 | **Tuning pass — restore the dialled-down variability layers (ADR 0027 P0, NO CODE)** | art-pipeline + owner | `Water.mat` sits 4–6× under the ADR-0010 design defaults: `_WindChop` 0.07/0.4, `_Octave3Weight` 0.108/0.3, `_FbmStrength` 0.045/0.18, `_FoamEvolveSpeed` 0.1/0.25; `_OceanSwellSharpness` 6/1.4 | **Free, and gates M3-23.** Separates "missing" from "turned off" before committing to the riskiest item. ⚠️ `_FbmScale` is **3.26 vs 0.05** — smaller = longer wavelength, so the "broad slow patches" layer runs at ~65× frequency; **needs an explicit owner ruling**, it is the one change that is not obviously safe. ⚠️ Do NOT tune `_Chop`/`_Roughness`/`_Flow` here — sim-pushed at runtime (§12.1) |
