...
Run these three, in this order. Each is re-runnable and harmless if the output already exists.

| Menu command | What it makes |
|---|---|
| **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸ Build Terrain Tiles** | The paintable ground tiles + an auto-shaping shoreline, under `Assets/_Project/Art/Tilesets/Tiles/`. |
| **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸ Build Tile Palette** | A palette called **HiddenHarboursTerrain** you pick tiles from, under `Assets/_Project/Art/Tilesets/Palettes/`. |
| **Hidden Harbours ▸ Art ▸ Build Decor Prefabs** | Drag-in trees, buildings, props, grass, wildflowers and the ten Acadian trees, under `Assets/_Project/Prefabs/Decor/`. |

Wait a few seconds after each. The Console (Window ▸ General ▸ Console) reports what was built.
...
1. **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸ Add Paintable Tilemap**.
...
**Hidden Harbours ▸ Dev ▸ Place Acadian Tree Lineup (all 10 species)**. It stands one of each in a
...
> **After a re-bake of the tree art**, re-run **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸
> Build Acadian Tree Prefabs**.
...
- **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸ Add Paintable Tilemap** → a
  `Grid`/`TerrainTilemap` canvas (it lands
...
| I want to… | Do this |
|---|---|
| Build/refresh the whole toolkit | **Art ▸ Import (after a new drop) ▸ Build Terrain Tiles**, then **▸ Build Tile Palette**, then **Art ▸ Build Decor Prefabs** — see **§0** |
...
| Get a surface to paint on | **Hidden Harbours ▸ Art ▸ Import (after a new drop) ▸ Add Paintable Tilemap** |
...
- Tile + palette + decor builders: `Assets/_Project/Art/Editor/` (`TileAssetBuilder`,
  `TilePaletteBuilder`, `DecorPrefabBuilder`, `PaintableTilemapMenu`) — all
