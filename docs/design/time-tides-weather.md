...
1. **Tide datum vs. art sea level.** We define tide as metres above chart datum, but the ¾ top-down camera shows water as a *plane*, not a side elevation. Need an agreed mapping from `tideHeight`/`waterDepth` → **visual cues** (shoreline waterline position, exposed-rock sprites toggling, flat tiles switching wet→dry, float heights). Coordinate with `art-and-audio-bible.md`. (Likely: per-region a small set of waterline states + tile wet/dry swaps keyed off `waterDepth` thresholds, rather than true vertical displacement.)
   - **Greybox resolution shipped (St Peters, gameplay-systems).** The flat-tile-wet/dry-swap idea above is now realised as the runtime **`TidalFlatVisual`** (`Code/Environment`): a grid of cells over the bar/flats whose colour is a pure function of the same `waterDepth = WaterLevel − terrainElevation` the walkability/boat-cross gate reads — **sand when exposed (`depth ≤ 0`), wet-sand at the waterline, shallow→deep blue as the column grows** — so the picture can never disagree with the gate (single source of truth, §3.5). This is the deliberately-simple placeholder; the **layered water shader** (gradient/wavy/specular/caustics/foam, depth-graduated) is the deferred art pass owned by art-pipeline (**ADR 0010**, M2/M3) and supersedes this tinting when it lands. The cell-colour mapping is `TidalFlatVisual.CellColor(...)`, EditMode-tested for the reveal + determinism.
