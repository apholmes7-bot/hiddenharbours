/* Hidden Harbours — parametric ISO Stern Trawler (M2 bake recipe, ADR-0006 — same pipeline as
   ...
   (three decks: house/bridge/below) are PUBLISHED for boatInteriorRig.js. Side doors stay dressed
   closed — not routes this pass.
   Exposes globalThis.SternTrawlerIso = { W,H,PX,DIRS,pivot,order,ROCK,rock(i),
   ...
   HULL,BOOT,CREAM,DECKF,WOOD,GLAS,BUFF,NET,STEEL,IRON,KEY } — pass 3 adds geometry,faces,doorFaces,LEVEL_IDS.
   ...
   nothing derived; bake via geometry().ids (TexCoord1.x). Gantry, warps, radar mast and stays are
   the DEDICATED `rigging` class so no cut can take a spar with a room. ASK A: geometry() publishes
   soleZ + ceilingZ (or an EXPLICIT open-above) per walkable level — and main_deck/house share one
   sole z (3.50), which is exactly the tie the published ceilings break. faces() hands the tagged
   static mesh to the extractor; doorFaces(opts) is the posed leaf (lv:'house');
   render(dir,{cullLevels:[...]}) is the reference cut. Outside the new fields the mesh, ramps and
   pixels are byte-identical to pass 2 — adjudicated in qa/Boat Cutaway QA.dc.html. */
(function (root) {
  const PX = 32, S = 32;
  ...
  (function build(){
    ...
    lv('rigging');                                // DEDICATED class — the gantry must survive every cut
  ...
  function render(dir, opts){
    ...
    if(opts.cutaway && root.BoatCutaway) fl=root.BoatCutaway.filter(fl, root.SternTrawlerIso, Object.assign({}, opts, {dir}));   // the depth-correct section — boatCutawayRig.js owns the rule set; absent → byte-identical
  ...
  // hull half-width at (y, z) and sheer height at y
  ...
  /* THE SHIP'S HOUSE, published — three decks for boatInteriorRig.js. */
  const HOUSE = {
  ...
  function geometry(){
    const D=HOUSE.decks;
    return {
      schema:'hidden-harbours/hull-geometry@1', hull:'sternTrawlerIsoRig', units:'m',
      frame:'+x stbd, +y bow, +z up; origin amidships, keel bottom, centreline',
      ids:Object.assign({}, LEVEL_IDS),
      riggingClass:'rigging — gantry, warps, radar mast, stays: tagged by CLASS, never welded to a cullable room',
      ...
      levels:[
        ...
        { id:'main_deck', deck:'main_deck', soleZ:DECK, ceilingZ:null,
          ceiling:{ kind:'open', lid:null, note:'the trawl deck is sky — the gantry crossing it is rigging, not a ceiling' } },
      ],
    };
  }
