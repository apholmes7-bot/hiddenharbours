(function(root){
  ...
  function materials(b,{finish=true}={}){
    ...
    return finish&&root.CharacterFinish?root.CharacterFinish.materials(out,b):out;
  }
  ...
  function create(key,{finish=true,headStudy=H}={}){
    ...
    let source=key==='fisher'?root.CharacterArtStudy.create({build,skeletonWorld:skeleton,bindMesh:bind.faces}):bind.faces;
    if(finish&&root.CharacterFinish)source=root.CharacterFinish.body(source,build,bind);
    const body=source.filter(f=>!oldHead.has(f.part)&&f.part!=='inseam');
    const headBuild={...build,headSize:R.propsOf(build).headK};
    const head=headStudy.createHead(headBuild,hc).map(f=>({...f,bone:f.v.map(()=>[[hi,1]])}));
    ...
    return {key,build,headBuild,faces,hi,bind,mats:materials(build,{finish}),colours:headStudy.colours(build),headStudy};
  }
