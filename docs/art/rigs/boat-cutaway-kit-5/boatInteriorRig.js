...
(function (root) {
  ...
  function wallGrid(out, spec, B, s){
    ...
    const us=[u0,u1].concat(spec.us||[]), zs=[z0,z1];
    ...
    const put=(ua,ub,za,zb,mat,b,db,noTex)=>{
      ...
      if(Math.hypot(n[0],n[1],n[2])<1e-7) return;                       // a cell the map collapsed (beyond a raked edge) paints nothing
  ...
  function buildShip(out, s, env, B){
    ...
    // the room's forward extent: a raked front is a PLANE — the sole runs to its foot and the side walls'
    // forward edge follows it (yF); y1 (its most-aft y) is where fitout stops so nothing pokes through the glass
    const yF=(z)=>(Fr ? yAtZ(z)+0.01 : Dk.y1-WT), y1s=yF(sole), y1c=yF(ceil);   // +0.01: the side walls run 3 cm through the front's plane so the corner has no rasterizer seam
    ...
    if(lux){
      ...
      slab(out,[[-hxAt(sole+1),y0],[hxAt(sole+1),y0],[hxAt(sole+1),y1s],[-hxAt(sole+1),y1s]],sole,'sole',-0.05);
      const hxs=hxAt(sole+1), bw=0.22;
      for(const seg of [[[-hxs,y0],[hxs,y0],[hxs,y0+bw],[-hxs,y0+bw]],[[-hxs,y1s-bw],[hxs,y1s-bw],[hxs,y1s],[-hxs,y1s]],
                        [[-hxs,y0],[-hxs+bw,y0],[-hxs+bw,y1s],[-hxs,y1s]],[[hxs-bw,y0],[hxs,y0],[hxs,y1s],[hxs-bw,y1s]]])
        slab(out,seg,sole+0.004,'wains',-0.30,plankTex(0.12));
    } else slab(out,[[-hxAt(sole+1),y0],[hxAt(sole+1),y0],[hxAt(sole+1),y1s],[-hxAt(sole+1),y1s]],sole,'sole',-0.10,plankTex(0.16));
    ...
    for(const side of [-1,1]){
      ...
      if(cullWall(outward,B)){
        ...
        quad(out,[xw,y0,sole+0.02],[xw,y1s,sole+0.02],[xw,y1s,sole+0.17],[xw,y0,sole+0.17],'cut',0.20,0.05);
        continue; }
      wallGrid(out,{ u0:y0-0.03,u1:Math.max(y1s,y1c), us:[y1s,y1c], z0:sole,z1:ceil, railZ, mat:'liner', railMat:'wains', tex:lux?null:boardTex(0.38),
        map:(y,z)=>[side*hxAt(z),Math.min(y,yF(z)),z], outward, cuts, b: side<0?0.16:-0.55 }, B, s);   // -0.03: through the aft wall's plane too — no corner seam
      quad(out,[side*hxAt(ceil),y0,ceil],[side*hxAt(ceil),y1c,ceil],[side*(hxAt(ceil)-0.20),y1c,ceil],[side*(hxAt(ceil)-0.20),y0,ceil],'ceil',-1.6,0.02);
    }
    ...
    for(const end of [{y:y0-0.0, outward:[0,-1,0], isDoor:((!isBridge && D.face==='aft')||(isBridge && !!Hh.door2 && !Dk.external)), glass:(isBridge?Dk.aftGlass:null), ports:(!isBridge&&Dk.aftPorts)?Dk.aftPorts:null, raked:false},
                      {y:y1s, outward:[0,1,0], isDoor:(!isBridge && D.face==='fwd'), glass:(isBridge&&Fr?Dk.frontGlass:null), ports:null, raked:!!(isBridge&&Fr)}]){
  ...
  function render(dir, opts){
    ...
    const bufs=paint(faces,B,MATS,s,env,set), rgba=toRGBA(post(bufs,s,env), env.L.cell);
    rgba.dep=bufs.dep;                       // depth rides along for the cutaway composite (boatCutawayRig.js)
    return rgba;
  }
