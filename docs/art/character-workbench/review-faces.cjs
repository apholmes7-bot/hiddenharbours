...
const contexts=['before','after'].map(facePass=>loadStudy({facePass}).context);
...
function render(ix,key,ppm,angle,expr='neutral',buildOverrides={}){
 ...
 return raster(faces,c.CastViewerEngine.materials(b),{w,h,cx:w/2+.5,cy:h/2,scale:ppm,angle,elev:40,mode:'proposal',outline:true,surface:(u,v)=>H.sample(u,v,state,b),surfaceColours:H.colours(b)});
}
function board(ppm){
